    ...
    try {
        ...
        if (mediaCount > 0) {
            ...
            for (const [indexInBatch, item] of mediaItems.entries()) {
                ...
                if ((blob || base64) && (isVideo || isImage)) {
                    if (isVideo && !isGif) {
...
/*else if (url) {
            ...
            ">
                    ...
                    if (active) {
                        element.classList.add('active');
                        await displayStoredData(element, dataBaseObjectStoreName);
                    }
                }*/
    ...
    try {
            ...
            return new Promise((resolve, reject) => {
                ...
                reader.onload = async (e) => {
                    try {
                        ...
                        // create a temporary object url and test if this environment can play it
                        const tempUrl = URL.createObjectURL(blob);
                        const canDisplay = await testBlobUrl(tempUrl, file.type).catch(() => false);
                        // revoke the temporary url used for testing
                        ...
                        reject(`Processing media failed: ${error.message || error}`);
                    }
                };
        ...
        for (const { id, timestamp, blob, base64Data, isVideo } of newMedia) {
            ...
            if (isVideo) {
                ...
                // store for later play call (and keep the objectURL so we don't revoke it prematurely)
                videosToPlay.push({ video, objectUrl: playbackSrc, id });

                // page-specific metadata logic
                if (dataBaseObjectStoreName === 'inputs' && pageName === 'face-swap') {
                    const keepFPS = document.getElementById('keepFPS');
                    const fpsSlider = document.getElementById('fps-slider');
                    const removeBanner = document.getElementById('removeBanner');
            ...
            // mark the newest uploaded item active (matching your original logic)
        ...
        // Try to play videos (handle autoplay promise)
        if (pageName !== 'face-swap') {
            videosToPlay.forEach(({ video, objectUrl }) => {
                try {
                    // ensure attributes/properties are set for mobile autoplay
                    video.controls = false;
                    video.preload = 'auto';
                    video.autoplay = true;
                    video.muted = true;
                    ...
                    // load and attempt to play, swallow promise rejection
                    video.load();
                    const playPromise = video.play();
                    if (playPromise && typeof playPromise.then === 'function') {
                        playPromise.catch(() => {
                            // autoplay prevented — it's fine; user can still tap to play
                        });
                    }
                } catch (e) {
                    // ignore play errors
...
export const setupFileUpload = async ({
    ...
    linkInput.addEventListener('keydown', async e => {
        ...
        await displayStoredData(wrapper, dataBaseObjectStoreName);
