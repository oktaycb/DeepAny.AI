...
window.isLowEndDevice = function () {
    ...
    const result = lowCpu || lowMemory || isMobile;
    return result;
};
...
let didCallStatus = false;

const storedVersion = localStorage.getItem('version') || '8.7.6.5';
...
export async function fetchPricingDataInBackground(serverAddressPayment) {
    try {
        ...
        console.error('Error while refreshing the pricing data: ' + error.message);
    }
}

export async function prefetchPricingServer() {
    if (pageName === 'pricing')
        return;
    const serverDocSnapshot = () => getDocSnapshot('servers', '3090-1');
    const [_serverAddressAPI, serverAddressPayment] = await Promise.all([
        fetchServerAddress(serverDocSnapshot, 'API'),
        fetchServerAddress(serverDocSnapshot, 'PAYMENTS'),
    ]);
    fetchPricingDataInBackground(serverAddressPayment);
    console.log('prefetchPricingServer completed.');
}
...
export async function setCurrentUserDoc(getDocSnapshot, useCache = false) {
    ...
    if (!userDoc) {
        console.warn('[setCurrentUserDoc] No user yet, waiting for auth...');
        userData = await waitForAuthUser();
    }

    if (!userData?.uid) {
        console.warn('[setCurrentUserDoc] User not logged in');
        return false;
    }
    ...
    setUser(userDocData);
    console.log('[setCurrentUserDoc] User document set');
    return true;
}
...
export function setUser(userDoc) {
    ...
    function setCachedImageForElements(className, storageKey) {
        ...
        const src = cachedImage || 'assets/profile.webp';
    ...
    if (credits) {
        ...
        const totalCredits = actualCredits + dailyCredits + rewardCredits;
        const creditText = totalCredits !== 0 ? totalCredits + ' Credits' : 'No Credits';
        ...
        if (deadlines.length) {
            const getDeadlineDate = d => new Date(d.seconds * 1000 + (d.nanoseconds || 0) / 1e6);
            const maxDeadlineDate = new Date(Math.max(...deadlines.map(d => getDeadlineDate(d))));
            const updateCountdown = () => {
                const now = new Date();
                let diff = maxDeadlineDate - now;

                if (diff <= 0) {
                    credits.textContent = creditText;
                ...
                if (years > 5) {
                    ...
                    let tempDiff = diff;
                    const y = Math.floor(tempDiff / year); tempDiff %= year;
                    const mo = Math.floor(tempDiff / month); tempDiff %= month;
                    const d = Math.floor(tempDiff / day); tempDiff %= day;
                    const h = Math.floor(tempDiff / hour); tempDiff %= hour;
                    const mi = Math.floor(tempDiff / minute); tempDiff %= minute;
                    ...
                    // Show minutes if display list is short, else show seconds
                    if (mi && parts.length < 2) parts.push(`${mi} minute${mi > 1 ? 's' : ''}`);
                    else if (!parts.length && s) parts.push(`${s} second${s > 1 ? 's' : ''}`);
                }

                credits.textContent = `${creditText} (${parts.join(', ')} remaining)`;
            };

            // Start the interval and run once immediately
            updateCountdown();
...
async function fetchWithTimeout(url, timeout, controller) {
    if (!controller) {
        ...
        try {
            const response = await fetchWithRandom(url, { signal });
            return response; // ⬅️ do NOT parse here
        } finally {
...
export async function getUserInternetProtocol() {
    ...
    try {
        for (let i = 0; i < urls.length; i++) {
            try {
                const data = await fetchWithTimeout(urls[i], 3000, controllers[i]);
...
export async function fetchAllServerAddresses(snapshotPromise, useCache = true) {
    ...
    const cacheKey = `cachedAllServerAddresses-${version}`;
    let cachedAddresses = getCache(cacheKey, ttl);

    if (cachedAddresses && (
        cachedAddresses.some(a => !a || a === 'undefined') ||
        cachedAddresses.length === 0
    )) {
        console.warn('[fetchAllServerAddresses] Invalid cache found → clearing');
        localStorage.removeItem(cacheKey);
        cachedAddresses = null;
    }

    if (useCache && cachedAddresses) {
        console.log('[fetchAllServerAddresses] cache hit');
...
export async function fetchServerAddresses(snapshotPromise, filter = true, serverType = null, useCache = true, pageNameCustom = null) {
    if (!pageNameCustom)
        pageNameCustom = pageName;
    const ttl = 7 * 24 * 60 * 60 * 1000;
    const cacheKey = `${pageNameCustom}-serverAddresses-${version}`;
    let cachedAddresses = getCache(cacheKey, ttl);
    ...
    if (cachedAddresses && (
        ...
        console.warn('[fetchServerAddresses] Invalid cache found → clearing');
        localStorage.removeItem(cacheKey);
        cachedAddresses = null;
    }

    if (useCache && cachedAddresses) {
        console.log('[fetchServerAddresses] cache hit');
        return cachedAddresses;
    }
    ...
    let serverAddresses = snapshot.docs
        .map(doc => {
            const data = doc.data();
            return data[serverType ? serverType : documentID()];
        })
        .filter(Boolean);

    if (!filter) {
        serverAddresses = snapshot.docs
            .map(doc => doc.data()[serverType ? serverType : documentID()])
            .filter(Boolean);
    }

    if (serverAddresses.length === 0) {
        console.warn('[fetchServerAddresses] No valid server addresses found');
        return [];
    }
...
export function showNotification(message, featureChange, type) {
    setTimeout(() => {
        ...
        const maxVisible = window.isMobileDevice() ? 1 : 3;
        ...
        if (currentNotifications.length > maxVisible) {
            const excess = currentNotifications.length - maxVisible;
            for (let i = 0; i < excess; i++) {
                const oldNotif = currentNotifications[i];
                oldNotif.style.opacity = '0';
                setTimeout(() => oldNotif.remove(), 250);
            }
        }
...
export const openMultipleDBs = async (configs) => {
    if (!Array.isArray(configs)) {
        throw new Error("openMultipleDBs expects an array");
    }

    const openPromises = configs.map(async ({ name, store }) => {
        if (!name || !store) {
            console.warn("Skipping invalid DB config:", { name, store });
            return null;
        }

        if (dbCache[name]) {
            return [name, dbCache[name]];
        }

        if (dbOpenPromises[name]) {
            const db = await dbOpenPromises[name];
            return [name, db];
        }

        dbOpenPromises[name] = openDB(name, store)
            .then(db => {
                dbCache[name] = db;         
                delete dbOpenPromises[name]; 
                return db;
            })
            .catch(err => {
                delete dbOpenPromises[name]; 
    ...
    const resolved = await Promise.all(openPromises);
    const results = {};
...
const dbDataCache = Object.create(null);
const dbDataPromises = Object.create(null);

export const getCachedFromDB = async (dbName, openFn) => {
    if (dbDataCache[dbName]) return dbDataCache[dbName];
    if (dbDataPromises[dbName]) return dbDataPromises[dbName];

    dbDataPromises[dbName] = (async () => {
        const db = await openFn();
        const data = await getFromDB(db);
        dbDataCache[dbName] = data;
        delete dbDataPromises[dbName];
        return data;
    })();

    return dbDataPromises[dbName];
};

export const closeAllDBs = () => {
    for (const name in dbCache) {
        try {
            dbCache[name].close();
        } catch { }
        delete dbCache[name];
...
export const openDB = (databaseName, objectStoreName, useCache = false) => {
    if (dbCache[databaseName] && useCache) 
        return Promise.resolve(dbCache[databaseName]);
    ...
    return new Promise((resolve, reject) => {
        ...
        // 2. String (url or base64)
            ...
            if (data.startsWith('http') || data.startsWith('/')) {
                ...
                return reject('addToDB: unsupported string format');
            }
        }

        // 3. Array [blob, url|base64]
        else if (Array.isArray(data)) {
            const [maybeBlob, maybeUrlOrBase64] = data;

            if (maybeBlob instanceof Blob) entry.blob = maybeBlob;

            if (typeof maybeUrlOrBase64 === 'string') {
                if (maybeUrlOrBase64.startsWith('http') || maybeUrlOrBase64.startsWith('/')) {
                    entry.url = maybeUrlOrBase64;
                } else if (maybeUrlOrBase64.startsWith('data:')) {
                    entry.base64 = maybeUrlOrBase64;
                } else {
                    return reject('addToDB: unsupported array string format');
                }
            }
        }

        // 4. Object payload (extended support)
        else if (typeof data === 'object' && data !== null) {
            if (data.blob instanceof Blob) entry.blob = data.blob;
            if (data.thumbnailBlob instanceof Blob) entry.thumbnailBlob = data.thumbnailBlob;

            if (typeof data.base64 === 'string') entry.base64 = data.base64;
            if (typeof data.thumbnailBase64 === 'string') entry.thumbnailBase64 = data.thumbnailBase64;

            if (typeof data.url === 'string') entry.url = data.url;
        }
        else {
            return reject('addToDB: unsupported data type');
        }
        ...
        req.onsuccess = () => resolve({ id: req.result, timestamp });
        req.onerror = e => {
            const err = e.target.error;
            alert(
                'Error adding to DB: ' +
                err +
                (String(err).includes('Blob')
                    ? ' | (possible private/incognito limitation)'
                    : '')
            );
            reject(err?.message || 'Unknown IDB error');
        };
    });
};
        ...
        try {
            ...
            request.onsuccess = (event) => {
                ...
                const mappedResults = results.map(item => ({
                    ...
                    thumbnailBase64: item.thumbnailBase64 || null,
                    id: item.id || null,
...
/*export const getFromDB = (db, limit = null) => {
    return new Promise((resolve, reject) => {
        const storeName = db.objectStoreNames[0];
    ...
    try {
        ...
        if (response.status === STATUS_OK) {
            ...
            if (onlineStatus) {
                ...
                //console.log('settings: ', `onlineStatus_${url.origin} = ${onlineStatus}`);
                localStorage.setItem(`onlineStatus_${url.origin}`, onlineStatus);
            }
...
async function displayStoredData(element, dataBaseObjectStoreName) {
    ...
    if (currentDisplayedElement[dataBaseObjectStoreName] === targetElement)
        return;

    if (element === 'loader') {
        document.querySelectorAll('[video-loader]').forEach(el => el.remove());
        ...
        loadingSpinner.setAttribute('video-loader', '');
        loadingSpinner.style.position = 'absolute';

        const beforeInputInit = document.getElementById(`${dataBaseObjectStoreName}_beforeInit`);
        if (beforeInputInit && beforeInputInit.parentElement) {
            ...
            try {
                ...
                if (isVideo) {
                    ...
                    const unmuteOnInteraction = () => {
                        fgClone.muted = false;
                        fgClone.removeEventListener('click', unmuteOnInteraction);
                        fgClone.removeEventListener('touchend', unmuteOnInteraction);
                    };
                    fgClone.addEventListener('click', unmuteOnInteraction, { once: true });
                    fgClone.addEventListener('touchend', unmuteOnInteraction, { once: true });
                ...
                const loaded = await awaitMediaLoadedWithRetries(fgClone);
                ...
                if (isLowEndDevice() && shouldCreateBG) {
                    ...
                    canvas.height = 16;
                    const ctx = canvas.getContext('2d');
                    ctx.drawImage(fgClone, 0, 0, canvas.width, canvas.height);

                    bgClone = document.createElement('img');
                    bgClone.src = canvas.toDataURL();
                ...
                if (!isLowEndDevice() && shouldCreateBG) {
                    if (!frameElement) {
                        ...
                        bgClone.style.height = '100%';
                        bgClone.style.objectFit = 'cover';
                        bgClone.style.filter = 'blur(16px)';
                        bgClone.style.transform = 'scale(1.1)';
                        bgClone.style.zIndex = '1';
                ...
                if (isVideo) {
                    fgClone.addEventListener('click', async () => {
                        try {
                            console.log(fgClone.paused);
                            if (fgClone.paused) {
                                fgClone.play();
                                if (bgClone instanceof HTMLVideoElement) {
                                    bgClone.play();
                                }
                            } else {
                                fgClone.pause();
                ...
                showNotification(
                    `Media could not be loaded. Retrying... (${attempt}/${maxRetries})`,
                    'Error',
                    'default'
                );

                if (attempt === maxRetries) {
                    showNotification('Failed to load media after multiple attempts.', 'Error', 'error');
                    return container; // maybe partially loaded
                }
    ...
    let clonedInput;
    try {
        clonedInput = await createClonedInput();
    } catch (err) {
        alert('An error occurred while loading media.');
    ...
    if (afterInitContainer) {
        afterInitContainer.innerHTML = '';
        afterInitContainer.appendChild(clonedInput);
    ...
    async function initInput() {
        return new Promise((resolve) => {
            ...
            const cleanup = () => {
                fgClone.removeEventListener('load', onLoad);
                fgClone.removeEventListener('loadeddata', onLoad);
                fgClone.removeEventListener('error', onError);
            };
            ...
            // Immediate check before waiting for events
            if (
                (isVideo && fgClone.readyState >= 2) ||
                (!isVideo && fgClone.complete && fgClone.naturalWidth !== 0)
            ) {
                onLoad();
                return;
            }

            fgClone.addEventListener(isVideo ? 'loadeddata' : 'load', onLoad, { once: true });
            fgClone.addEventListener('error', onError, { once: true });
        });
    }

    const hasError = await initInput();
    if (hasError) {
        alert('Media initialization failed.');
        return;
    }
    ...
    if (isVideo) {
        function generateTimeline() {
            ...
            if (afterInitContainer) afterInitContainer.appendChild(timelineContainer);

            const isInpaintMode = pageName === 'inpaint' && fgClone.getAttribute("data-fps");
            ...
            function createHandle(name, color) {
                ...
                return { el: handle, tooltip, time: 0, dragging: false };
            }

            const handles = {
                left: createHandle(`${dataBaseObjectStoreName}_leftClampHandle`, "red"),
                mid: createHandle(`${dataBaseObjectStoreName}_midHandle`, "green"),
                right: createHandle(`${dataBaseObjectStoreName}_rightClampHandle`, "red")
            };

            // Show/hide left and right handles based on mode
            if (!isInpaintMode) {
                handles.left.el.style.display = 'none';
                handles.right.el.style.display = 'none';
            }

            rail.appendChild(handles.left.el);
            rail.appendChild(handles.mid.el);
            rail.appendChild(handles.right.el);
            ...
            function updateTooltip(h) {
                h.tooltip.innerText = `${formatTime(h.time)}/${formatTime(videoDuration)}`;
            }

            function updateAllTooltips() {
                updateTooltip(handles.left);
                updateTooltip(handles.mid);
                updateTooltip(handles.right);
            }

            function updateVideoAttributes() {
                targetElement.setAttribute("data-first-time", Math.round(handles.left.time));
                targetElement.setAttribute("data-last-time", Math.round(handles.right.time));
                targetElement.setAttribute("data-current-time", Math.round(handles.mid.time));
            }

            function timeToPx(t) {
                const rect = rail.getBoundingClientRect();
                return videoDuration > 0 && rect.width > 0 ? (t / videoDuration) * rect.width : 0;
            }

            function pxToTime(px) {
                const rect = rail.getBoundingClientRect();
                return videoDuration > 0 && rect.width > 0 ? (px / rect.width) * videoDuration : 0;
            }

            function clampTimes() {
                if (isInpaintMode) {
                    // Always maintain exactly 5s gap between left and right handles for inpaint mode
                    if (handles.left.dragging) {
                        handles.right.time = Math.min(videoDuration, handles.left.time + 5);
                    } else if (handles.right.dragging) {
                        handles.left.time = Math.max(0, handles.right.time - 5);
                    } else {
                        // No dragging - ensure gap is exactly 5 seconds (or video duration if shorter)
                        const maxGap = Math.min(5, videoDuration);
                        const currentGap = handles.right.time - handles.left.time;
                        if (Math.abs(currentGap - maxGap) > 0.01) {
                            handles.right.time = handles.left.time + maxGap;
                        }
                    }

                    // Clamp to video boundaries for inpaint mode
                    if (handles.left.time < 0) {
                        handles.left.time = 0;
                        handles.right.time = Math.min(5, videoDuration);
                    }
                    if (handles.right.time > videoDuration) {
                        handles.right.time = videoDuration;
                        handles.left.time = Math.max(0, videoDuration - Math.min(5, videoDuration));
                    }
                } else {
                    // Normal mode - just clamp to boundaries without gap logic
                    if (handles.left.time < 0) handles.left.time = 0;
                    if (handles.right.time > videoDuration) handles.right.time = videoDuration;
                }

                // Middle handle follows video currentTime when not dragging
                if (!handles.mid.dragging) {
                    handles.mid.time = fgClone.currentTime;
                }

                // Clamp middle handle inside the range
                if (handles.mid.time < handles.left.time) handles.mid.time = handles.left.time;
                if (handles.mid.time > handles.right.time) handles.mid.time = handles.right.time;
            }

            const frameTextContainer = document.createElement("div");
            frameTextContainer.style.position = "absolute";
            frameTextContainer.style.bottom = "calc(100% + 6px)";
            frameTextContainer.style.left = "50%";
            frameTextContainer.style.transform = "translateX(-50%)";
            frameTextContainer.style.color = "white";
            frameTextContainer.style.fontSize = "1vh";
            frameTextContainer.style.display = "flex";
            frameTextContainer.style.gap = "1vh";
            frameTextContainer.style.bottom = "4.5vh";
            frameTextContainer.style.zIndex = "55";
            timelineContainer.appendChild(frameTextContainer);

            const firstFrameText = document.createElement("span");
            const currentFrameText = document.createElement("span");
            const lastFrameText = document.createElement("span");

            [firstFrameText, currentFrameText, lastFrameText].forEach(span => {
                span.style.background = "rgba(255, 255, 255, 0.9)";
                span.style.color = "black";
                span.style.padding = "0.1vh 0.3vh";
                span.style.borderRadius = "2px";
                span.style.fontSize = "1vh";
            });

            frameTextContainer.appendChild(firstFrameText);
            frameTextContainer.appendChild(currentFrameText);
            frameTextContainer.appendChild(lastFrameText);

            const fps = parseFloat(fgClone.getAttribute("data-fps")) || 30;

            function updateFrameTexts() {
                // Only show frame texts in inpaint mode
                if (isInpaintMode) {
                    const firstFrame = Math.round(handles.left.time * fps);
                    const currentFrame = Math.round(handles.mid.time * fps);
                    const lastFrame = Math.round(handles.right.time * fps);

                    firstFrameText.innerText = `First: ${firstFrame}`;
                    currentFrameText.innerText = `Current: ${currentFrame}`;
                    lastFrameText.innerText = `Last: ${lastFrame}`;
                    frameTextContainer.style.display = "flex";
                } else {
                    frameTextContainer.style.display = "none";
                }
            }

            function renderPositions() {
                handles.left.el.style.left = `${timeToPx(handles.left.time)}px`;
                handles.mid.el.style.left = `${timeToPx(handles.mid.time)}px`;
                handles.right.el.style.left = `${timeToPx(handles.right.time)}px`;
                updateAllTooltips();
                ...
                if (!isInpaintMode) {
                    ...
                    handles.left.el.style.display = 'flex';
                    handles.right.el.style.display = 'flex';
                }
            }

            function attachDrag(h, step) {
                ...
                function onPointerMove(e) {
                    ...
                    let newTime = pxToTime(clientX - rect.left);
                ...
                h.el.addEventListener("touchstart", onPointerDown, { passive: false });
            }

            attachDrag(handles.left, "second");
            attachDrag(handles.right, "second");
            attachDrag(handles.mid, "frame");
            ...
            handles.mid.time = (handles.left.time + handles.right.time) / 2;
            ...
            timelineContainer.addEventListener("click", (e) => {
                ...
                if (!isInpaintMode) {
                    ...
                    if (time < handles.left.time) time = handles.left.time;
                    if (time > handles.right.time) time = handles.right.time;
                }

                handles.mid.time = time;
                fgClone.currentTime = time;
                bgClone.currentTime = time;
            ...
            fgClone.addEventListener("play", () => {
                if (fgClone.currentTime < handles.left.time || fgClone.currentTime >= handles.right.time) {
                    fgClone.currentTime = handles.left.time;
                    bgClone.currentTime = handles.left.time;
                }
            });
            ...
            fgClone.addEventListener("timeupdate", function () {
                ...
                if (fgClone.currentTime >= handles.right.time) {
                    if (!fgClone.paused) {
                        fgClone.pause();
    ...
    currentDisplayedElement[dataBaseObjectStoreName] = targetElement;
}
...
export const handleFileContainerEvents = async (event, dataBaseIndexName, dataBaseObjectStoreName, container, databases) => {
    ...
    if (now - lastClickTime < 25) {
        const seconds = ((now - lastClickTime) / 1000).toFixed(1);
        showNotification(
            `Please wait before clicking again (${seconds}s).`,
            'Loading in progress',
            'warning'
        );
        return;
    ...
    let parent = event.target.closest('.data-container');
    if (!parent) return;
    if (event.target.classList.contains('settings-mark')) return;
    if (event.target.classList.contains('delete-icon')) return await handleDelete(dataBaseIndexName, dataBaseObjectStoreName, parent, container, databases);

    // Use per-store loading lock
    if (isMediaLoading[dataBaseObjectStoreName] === true) {
        showNotification('Please wait until current media finishes loading.', 'Loading in progress', 'warning');
        return;
    }
    ...
    let element = parent.querySelector('img, video, iframe, initial');
    if (!element) return;

    const timestamp = element?.getAttribute('timestamp');
    const activate = document.getElementById(`${timestamp}_activate`);
    if (activate)
        simulateFullClick(activate);
};
...
export function handleContextMenu(div) {
  ...
  <div class="background-dot-container overflow-hidden">
    <div class="background-dot-container-content" style='overflow: auto;justify-content: space-between;padding-right: calc(1vh * var(--scale-factor-h));'>
      ...
      <div class="small-box unreversed" data-action="open" id="${timestamp}_open">
        <span class="menu-icon">↗</span>
        <span>Open</span>
      </div>
      <div class="small-box unreversed" data-action="activate" id="${timestamp}_activate">
        <span class="menu-icon">↗</span>
        <span>Activate</span>
      </div>
      <div class="small-box unreversed" data-action="delete" id="${timestamp}_delete">
        <span class="menu-icon">×</span>
        <span>Delete</span>
      </div>
      <div class="small-box unreversed" data-action="save" id="${timestamp}_save">
        <span class="menu-icon">↓</span>
        <span>Save</span>
      </div>
      <div class="small-box unreversed" data-action="download-browser" id="${timestamp}_download-browser">
        …
      </div>
      <div class="small-box unreversed" data-action="download-server" id="${timestamp}_download-server">
        …
      </div>
      <div class="small-box unreversed" data-action="copy-browser-url" id="${timestamp}_copy-browser-url">
        …
      </div>
      <div class="small-box unreversed" data-action="copy-download-url" id="${timestamp}_copy-download-url">
        …
      </div>
      <div class="small-box unreversed" data-action="use-generation-data" id="${timestamp}_use-generation-data">
        …
      </div>
    </div>
  </div>
...
`;
    ...
    contextMenu.addEventListener('touchmove', (e) => {
        const scrollArea = contextMenu.querySelector('.background-dot-container-content');

        if (!scrollArea) return;

        const isInsideScrollArea = scrollArea.contains(e.target);
        const canScroll = scrollArea.scrollHeight > scrollArea.clientHeight;

        if (isInsideScrollArea && canScroll) {
            return;
        }
    ...
    div.addEventListener('click', (e) => {
        const sm = e.target.closest('.settings-mark');
        if (!sm) return;
    ...
    contextMenu.addEventListener('click', async (e) => {
        ...
        if (!item) {
            ...
            item = items.find(item => item.timestamp === domTimestamp);
        }
        ...
        if (!item) {
            showNotification('Could not find media data.', 'Error', 'error');
            return;
        }

        let action = menuItem.getAttribute('data-action');
        let url = element?.querySelector('source')?.src || element?.src;
        let downloadUrl = element?.getAttribute('url');

        // Use per-store loading lock
        if (isMediaLoading[parentParentClass]) {
            showNotification('Please wait until current media finishes loading.', 'Loading in progress', 'warning');
            return;
        }
        ...
        if (!element.parentElement.classList.contains('active')) {
            if (item?.blob) {
                try {
                    console.log('Loading media element:', item);
                    isMediaLoading[parentParentClass] = true;
                    //element = await replaceElementForData(element, parentParentClass, item);
                    await loadMedia(element, item?.blob);
                } catch (err) {
                    console.error('Media failed to load:', err);
                } finally {
                    isMediaLoading[parentParentClass] = false;
                }
            }
        }
        ...
        switch (action) {
            case 'activate':
                ...
                const activeEls = [...element.parentElement.parentElement.querySelectorAll('.data-container.active')];
                for (const activeEl of activeEls) {
                    const activeElement = activeEl.querySelector('img, video, iframe, initial');
                    if (activeElement !== element) {
                        activeEl.classList.remove('active');

                        if (activeElement?.tagName.toLowerCase() === 'iframe') {
                            activeElement.style.width = 'calc(var(--input-size))';
                            activeElement.style.height = 'calc(var(--input-size))';
                        }

                        if (activeElement?.src?.startsWith('blob:')) {
                            if (activeElement.tagName.toLowerCase() === 'video') activeElement.pause();
                            URL.revokeObjectURL(activeElement.src);
                        }

                        const idx = parseInt(activeElement?.getAttribute('id'));
                        if (!isNaN(idx)) {
                            (function () {
                                (async () => {
                        ...
                        if (pageName === 'inpaint' || pageName === 'art-generator' || pageName === 'face-swap' || dataBaseObjectStoreName === 'outputs')
                            displayStoredData(element.parentElement, pageName === 'inpaint' ? 'outputs' : parentParentClass);
                    }
                }
            ...
            case 'open':
                ...
                if (pageName === 'inpaint' || pageName === 'art-generator' || pageName === 'face-swap' || dataBaseObjectStoreName === 'outputs')
                    displayStoredData(parent, pageName === 'inpaint' ? 'outputs' : parentParentClass);
                const ok = window.confirm("Do you want to open this in a new tab?");
                if (ok) {
            ...
            case 'save':
                ...
                if (element) {
                    ...
                    const fileName = 'output_' + `${id}.${element.tagName.toLowerCase() === "video" ? 'mp4' : 'png'}`;
    ...
    function restoreConfig(config) {
        ...
        document.querySelectorAll('input[type="range"]').forEach(sl => {
            ...
            const parentSlider = sl.closest('.slider');
            if (parentSlider && parentSlider !== sl && parentSlider.id) {
                key = parentSlider.id;
                container = parentSlider;
            }
            ...
            if (!key && sl.dataset && sl.dataset.key) {
                key = sl.dataset.key;
                container = sl;
            }

            if (!key && sl.parentElement && sl.parentElement.id) {
                key = sl.parentElement.id;
                container = sl.parentElement;
            }
            ...
            let display = null;
            try {
                if (container && container !== sl) display = container.querySelector('.slider-value');
            } catch (e) { }

            if (!display) {
                const next = sl.nextElementSibling;
                const prev = sl.previousElementSibling;
                if (next && next.classList && next.classList.contains('slider-value')) display = next;
                else if (prev && prev.classList && prev.classList.contains('slider-value')) display = prev;
            }

            if (display) {
                const num = Number(val);
                display.textContent = Number.isFinite(num) ? num.toFixed(2) : String(val);
            }
        ...
        if (typeof config.model === 'string') {
            ...
            if (container) {
                container.querySelectorAll('.rectangle').forEach(rect => {
                    const checkbox = rect.querySelector(`input[type="checkbox"][id="${modelId}"]`);
                    const active = checkbox !== null;

                    rect.classList.toggle('selected', active);
    ...
    function extractGenerationDataFromPNG(bytes) {
        // bytes: Uint8Array
        ...
        while (pos + 8 <= bytes.length) {
            ...
            if (type === 'tEXt') {
                ...
                if (nulIndex !== -1) {
                    const key = decoder.decode(chunkData.subarray(0, nulIndex));
                    const value = decoder.decode(chunkData.subarray(nulIndex + 1));
                    textChunks[key] = value;
                }
            } else if (type === 'iTXt') {
                const chunkData = bytes.subarray(dataStart, dataEnd);
                // iTXt: key\0 compFlag(1) compMethod(1) lang\0 trans\0 text
                let i = 0;
                const keyEnd = chunkData.indexOf(0, i);
                if (keyEnd === -1) { pos = dataEnd + 4; continue; }
                const key = decoder.decode(chunkData.subarray(i, keyEnd));
                i = keyEnd + 1;
                if (i + 2 > chunkData.length) { pos = dataEnd + 4; continue; }
                const compFlag = chunkData[i++];
                const compMethod = chunkData[i++];
                // find lang tag end
                const langEnd = chunkData.indexOf(0, i);
                if (langEnd === -1) { pos = dataEnd + 4; continue; }
                i = langEnd + 1;
                const transEnd = chunkData.indexOf(0, i);
                if (transEnd === -1) { pos = dataEnd + 4; continue; }
                i = transEnd + 1;
                // Only support uncompressed (compFlag === 0)
                if (compFlag === 0) {
                    const text = decoder.decode(chunkData.subarray(i));
                    textChunks[key] = text;
                } else {
                    // compressed iTXt (rare) - not supported here
                    console.warn('Compressed iTXt found - skipping');
                }
            }
        ...
        if (!textChunks.generationData) return null;
        try {
            return JSON.parse(textChunks.generationData);
        } catch (err) {
            console.error('generationData exists but JSON parse failed', err);
            return null;
        }
    }
    ...
    window.addEventListener('drop', async e => {
        ...
        // prefer text drops (you already had this)
        ...
        if (text) {
            try {
                ...
                showNotification(
                    `We have restored your configuration from text data.`,
                    'Copy Generation Data',
                    'notify'
                );
                return;
            } catch (err) {
                console.error('Dropped text is not valid JSON');
            }
        }
        ...
        try {
            ...
            const config = extractGenerationDataFromPNG(u8);
            if (!config) {
                console.error('No generationData metadata found');
                return;
            }
            console.log('Restored config:', config);
            restoreConfig(config);
            showNotification(
                `We have restored your configuration from image data.`,
                'Copy Generation Data',
                'notify'
            );
        } catch (err) {
...
/*async function cleanupLocalStorage(snapshotPromise) {
    ...
    function isServerValid(serverBase) {
        const serverBaseLower = serverBase.toLowerCase();
        const valid = allServers.some(s => serverBaseLower.includes(s.toLowerCase()));
        return valid;
    }

    function getAllDownloadOutputKeys() {
        const keys = [];
        for (const key of Object.keys(localStorage)) {
            if (key.includes("/download-output/") && key.endsWith("_formData")) {
                const base = key.replace("_formData", "");
                keys.push({ fullKey: key, baseUrl: base });
            }
        }
        return keys;
    }

    const downloadItems = getAllDownloadOutputKeys();

    let removedCount = 0;
    let skippedCount = 0;

    for (const item of downloadItems) {
        const serverBase = item.baseUrl.split("/download-output/")[0];

        if (!isServerValid(serverBase)) {
            console.log(`[cleanup] Removing ${item.fullKey} (server not in allServers list)`);
    ...
    console.log(`[cleanup] Cleanup finished. Removed: ${removedCount}, Kept/Skipped: ${skippedCount}`);
}

async function cleanupStaleDownloadOutput(serverList) {

    function isServerInUse(serverList, serverBase) {
        const serverBaseLower = serverBase.toLowerCase();
        const inUse = serverList.some(s => serverBaseLower.startsWith(s.toLowerCase()));
        return inUse;
    }

    function getDisplayedDownloadOutputUrls() {
        const urls = new Set();

        document.querySelectorAll('[url]').forEach(el => {
            const url = el.getAttribute('url');
            if (url && url.includes("/download-output/")) {
                urls.add(url);
            }
        });
        ...
        document.querySelectorAll('img[src], video[src]').forEach(el => {
            ...
            if (src && src.includes("/download-output/")) {
                urls.add(src);
            }
        });
    ...
    const downloadItems = getAllDownloadOutputKeys();
    const displayedUrls = getDisplayedDownloadOutputUrls();
    ...
    for (const item of downloadItems) {
        ...
        if (!isServerInUse(serverList, serverBase)) {
            console.log(`[cleanup] Skipping ${item.fullKey} (server not in use)`);
            skippedCount++;
            continue;
        }

        if (!displayedUrls.has(item.baseUrl)) {
            localStorage.removeItem(item.fullKey);
            removedCount++;
        } else {
            skippedCount++;
...
let loadingSpinnerInitWrapper = null;
let skipBtn = null;
let loadingSpinnerInit = null;
let activeDBInits = 0;
...
export const initDB = async (dataBaseIndexName, dataBaseObjectStoreName, handleDownload, databases, cacheAvailable = false) => {
    //console.time(`db-await_${dataBaseObjectStoreName}`);
    try {
        ...
        if (!cacheAvailable) {
            ...
            if (mediaCount > 0) {
                ...
                for (let i = 0; i < mediaCount; i++) {
                    ...
                    div.innerHTML = `<div class="process-text">Loading...</div><div class="delete-icon"></div><div class="settings-mark"></div>`;
                ...
                if (activeDBInits === 0) {
                    ...
                    loadingSpinnerInitWrapper = document.createElement('div');
                    loadingSpinnerInitWrapper.id = 'loadingSpinnerInitWrapper';
                    loadingSpinnerInitWrapper.style.position = 'fixed';
                    loadingSpinnerInitWrapper.style.top = '0';
                    loadingSpinnerInitWrapper.style.left = '0';
                    loadingSpinnerInitWrapper.style.width = '100%';
                    loadingSpinnerInitWrapper.style.height = '100%';
                    loadingSpinnerInitWrapper.style.display = 'flex';
                    loadingSpinnerInitWrapper.style.flexDirection = 'column';
                    loadingSpinnerInitWrapper.style.justifyContent = 'center';
                    loadingSpinnerInitWrapper.style.alignItems = 'center';
                    loadingSpinnerInitWrapper.style.zIndex = '99999';
                    loadingSpinnerInit = document.createElement('div');
                    loadingSpinnerInit.className = 'loading-screen';
                    loadingSpinnerInit.id = 'loadingSpinnerInit';

                    skipBtn = document.createElement('button');
                    skipBtn.innerText = "Skip Loading Screen...";
                    skipBtn.style.position = 'absolute';
                    skipBtn.style.bottom = '40vh';
                    skipBtn.style.width = '15%';

                    skipBtn.onclick = () => {
                        const w = document.getElementById('loadingSpinnerInitWrapper');
                        if (w) w.remove();
                        document.querySelectorAll('body main').forEach(main => {
                            main.style.filter = '';
                        });
                    ...
                    loadingSpinnerInitWrapper.appendChild(loadingSpinnerInit);
                    loadingSpinnerInitWrapper.appendChild(skipBtn);
                    document.documentElement.appendChild(loadingSpinnerInitWrapper);
                }
                activeDBInits++;
            }
            ...
            //console.timeEnd(`db-await_${dataBaseObjectStoreName}`);

            if (mediaCount > 0) {
                const mediaItems = await getCachedFromDB(
                    dataBaseIndexName,
                    () => openDB(dataBaseIndexName, dataBaseObjectStoreName)
                );
                const inputElements = mediaContainer.querySelectorAll('.data-container');
                //mediaItems.reverse();
                for (const [indexInBatch, item] of mediaItems.entries()) {
                    const { thumbnailBlob, thumbnailBase64, url, id, timestamp, active } = item;

                    // Keep the original media blob separate from the display blob (thumbnail fallback).
                    const originalBlob = item.blob || null;
                    const displayBlob = originalBlob && (active || !thumbnailBlob) ? originalBlob : thumbnailBlob;
                    const displayBase64 = item.base64 && (active || !thumbnailBase64) ? item.base64 : thumbnailBase64;

                    // helper to convert blob -> src (objectURL preferred, fallback to base64)
                    async function blobToSrc(blobObj) {
                        ...
                        try {
                            ...
                            // fallback to base64
                            const b64 = await blobToBase64(blobObj);
                            const fallbackMime = blobObj.type || 'application/octet-stream';
                    ...
                    // compute src and mime for display blob (thumbnail or media depending on active) and for original media blob
                    let srcDisplay = null;
                    let mimeDisplay = null;
                    let srcMedia = null;
                    let mimeMedia = null;

                    try {
                        // Display blob -> srcDisplay, mimeDisplay
                        if (displayBlob) {
                            mimeDisplay = displayBlob.type || null;
                            if (!mimeDisplay) {
                                const detected = await detectMimeFromBlob(displayBlob);
                                if (detected) mimeDisplay = detected;
                            }
                            srcDisplay = await blobToSrc(displayBlob);
                        } else if (displayBase64 && typeof displayBase64 === 'string' && displayBase64.startsWith('data:')) {
                            srcDisplay = displayBase64;
                            const idx = displayBase64.indexOf(';');
                            if (idx > 5) mimeDisplay = displayBase64.slice(5, idx);
                        } else if (url) {
                            srcDisplay = url;
                            mimeDisplay = null;
                        }

                        // Original media blob -> srcMedia, mimeMedia (if present)
                        if (originalBlob) {
                            mimeMedia = originalBlob.type || null;
                            if (!mimeMedia) {
                                const detected = await detectMimeFromBlob(originalBlob);
                                if (detected) mimeMedia = detected;
                            }
                            // only create media src now — we might not set as video.src if not active
                            srcMedia = await blobToSrc(originalBlob);
                        } else {
                            // if no original blob, maybe url points to the media
                            srcMedia = url || null;
                        }
                    } catch (e) {
                        srcDisplay = srcDisplay || null;
                        mimeDisplay = mimeDisplay || null;
                        srcMedia = srcMedia || null;
                        mimeMedia = mimeMedia || null;
                    }

                    // if mime still null but src exists, try to guess from src extension (best-effort)
                    function guessMimeFromUrl(u) {
                        if (!u || typeof u !== 'string') return null;
                        const extMatch = u.split('?')[0].match(/\.([a-zA-Z0-9]+)$/);
                    ...
                    if (!mimeDisplay && srcDisplay) {
                        const guessed = guessMimeFromUrl(srcDisplay);
                        if (guessed) mimeDisplay = guessed;
                    }
                    if (!mimeMedia && srcMedia) {
                        const guessed = guessMimeFromUrl(srcMedia);
                        if (guessed) mimeMedia = guessed;
                    }

                    const isGifDisplay = mimeDisplay === 'image/gif' || (displayBlob && displayBlob.type === 'image/gif');
                    // Decide whether the *original media* is video (important change: base detection on originalBlob / mimeMedia / url extension)
                    const isVideoMedia = (mimeMedia && mimeMedia.startsWith('video')) ||
                        (originalBlob && originalBlob.type && originalBlob.type.startsWith('video')) ||
                        (!originalBlob && srcMedia && (/\.mp4$|\.mov$|\.webm$|\.m4v$/i).test(srcMedia || ''));

                    const isImageDisplay = (mimeDisplay && mimeDisplay.startsWith('image')) || (displayBlob && displayBlob.type && displayBlob.type.startsWith('image'));

                    // If srcMedia is missing but originalBlob exists, try objectURL fallback (blobToSrc already attempted)
                    if (!srcMedia && originalBlob) {
                        try { srcMedia = await blobToSrc(originalBlob); }
                        catch (e) { srcMedia = null; }
                    }

                    // ensure we have a container
                    let container = inputElements[indexInBatch];
                    if (!container) {
                        container = document.createElement('div');
                    ...
                    // If media is a video (originally), create a <video> element even if display blob is an image thumbnail.
                    if (isVideoMedia) {
                        // Add tooltip for inputs same as original code
                        ...
                        video.setAttribute('active', active ? 'true' : 'false');
                        // If not active, mark as resized to indicate we're not loading full size / playing yet
                        video.setAttribute('resized', !active ? 'true' : 'false');
                        video.id = id;
                        video.preload = "auto";
                        video.muted = true;
                        ...
                        // If we have a thumbnail (display blob) that is an image and the item is NOT active, use it as poster.
                        if (!active && srcDisplay && isImageDisplay) {
                            // use poster instead of setting src (prevents heavy download)
                            try {
                                video.poster = srcDisplay;
                                // if poster is an objectURL we should revoke it after it has been used.
                                if (displayBlob && srcDisplay.startsWith('blob:')) {
                                    // create an Image to detect when poster has been loaded so we can revoke
                                    const imgLoader = new Image();
                                    imgLoader.onload = () => {
                                        try { URL.revokeObjectURL(srcDisplay); } catch (e) { }
                                    };
                                    imgLoader.onerror = () => {
                                        try { URL.revokeObjectURL(srcDisplay); } catch (e) { }
                                    };
                                    imgLoader.src = srcDisplay;
                                }
                            } catch (e) {
                                console.warn('Setting poster failed:', e);
                            }
                        } else {
                            // If active (or no image thumbnail), set the video src so it loads/display normally.
                            if (srcMedia) {
                                try { video.src = srcMedia; }
                                catch (e) { /* ignore */ }
                            } else if (url) {
                        ...
                        // If inputs store, compute metadata as before but only if the video actually has a source to load
                        if (dataBaseObjectStoreName === 'inputs') {
                            ...
                            // Only call calculateMetadata if the element has a source that can load metadata
                            if (video.src) {
                        ...
                        // If video was set from an objectURL (srcMedia) and it's not active (we set src only if active),
                        // we revoke objectUrl on loadeddata once it has been used. If we set src and used an objectURL, revoke afterwards.
                        if (video.src && video.src.startsWith && video.src.startsWith('blob:')) {
                            video.addEventListener('loadeddata', () => {
                                //try { URL.revokeObjectURL(video.src); } catch (e) { }
                            }, { once: true });
                            try { video.load(); video.pause(); } catch (e) { }
                        } else {
                            // if no src (we're showing poster only), do not attempt to load video
                        }

                        // Mark active state and display stored data as original code
                        if (active) {
                            container.classList.add('active');
                            if (pageName === 'face-swap' || dataBaseObjectStoreName === 'outputs')
                                displayStoredData(container, dataBaseObjectStoreName);
                        }
                    } else if (srcDisplay && (isImageDisplay || isGifDisplay)) {
                        // It's an image / gif display (original media not video).
                        const img = document.createElement('img');
                        img.setAttribute('url', url || '');
                        img.id = id;
                        img.setAttribute('timestamp', timestamp || '');
                        img.setAttribute('active', active ? 'true' : 'false');
                        img.setAttribute('resized', !active ? 'true' : 'false');
                        img.alt = isGifDisplay ? 'Loading...' : 'Loading...';
                        if (srcDisplay) img.src = srcDisplay;
                        ...
                        // Fallback: url-only handling (same as original)
                        container.innerHTML = `<initial url="${url}" id="${id}" timestamp="${timestamp}" active="${active}"/></initial><div class="process-text">Fetching...</div><div class="delete-icon"></div><div class="settings-mark"></div>`;
                        (async () => {
                            try {
                                if (container.isConnected) {
                                    ...
                                    const data = await Promise.race([
                                        fetchProcessState(url),
                                        timeout(10000)
                                    ]);

                                    if (data?.status === 'completed') {
                                        container.querySelector('.process-text').textContent = 'Downloading...';

                                        try {
                                            await handleDownload(
                                                { db, url, element: container, id, timestamp, active },
                                                databases
                                            );
                                        } catch (downloadErr) {
                                            console.error("handleDownload failed:", downloadErr);
                                            if (container.isConnected) {
                                                container.querySelector('.process-text').textContent = 'Download Failed';
                                            }
                                        }
                                    } else {
                                        if (container.isConnected) {
                                            container.querySelector('.process-text').textContent =
                                                data?.server || 'Unkown...';
                                        }
                                    }
                                }
                            } catch (err) {
                                console.error("fetchProcessState failed:", err);
                                if (container.isConnected) {
                                    container.querySelector('.process-text').textContent = 'Error';
                                }
                            }
                        })();
                    } else {
                    ...
                    // --- THUMBNAIL CREATION / PERSISTENCE (unchanged except using originalBlob variable) ---
                    // If originalBlob exists but there is no thumbnailBlob persisted yet, create & persist one.
                    if (originalBlob && !thumbnailBlob) {
                        try {
                            ...
                            if (thumb) {
                                if (url) {
                                    try {
                                        await updateInDB(db, url, originalBlob, thumb);
                                    } catch (updErr) {
                                        if (id != null) {
                                            try {
                                                await new Promise((resolve, reject) => {
                                                    ...
                                                    getReq.onsuccess = () => {
                                                        ...
                                                        if (rec) {
                                                            rec.thumbnailBlob = thumb;
                                                            rec.blob = originalBlob;
                                                            rec.chunks = [];
                                                            const putReq = store.put(rec);
                                                            putReq.onsuccess = () => resolve();
                                                            putReq.onerror = (ev) => reject(ev.target.error);
                                                        } else {
                                                            reject(new Error('Record not found for id ' + id));
                                                        }
                                                    };
                                                    getReq.onerror = (ev) => reject(ev.target.error);
                                                });
                                            } catch (e) {
                                                console.warn('Failed to update thumbnail by id:', e);
                                            }
                                        } else {
                                            console.warn('Could not persist thumbnail: no url and no id.');
                                        }
                                    }
                                } else if (id != null) {
                                    try {
                                    ...
                                    console.warn('Thumbnail created but could not be persisted: missing url & id');
                                }
                            }
                        } catch (err) {
                            console.error('createThumbnail failed in initDB:', err);
                        }
                    }
                }
        ...
        if (startedDBInits === 0) {
            ...
            if (loadingSpinnerInitWrapper)
                loadingSpinnerInitWrapper?.remove();
            if (loadingSpinnerInit)
                loadingSpinnerInit?.remove();
            if (skipBtn)
                skipBtn?.remove();
            document.querySelectorAll('body main').forEach(main => main.style.filter = 'none');

            //const snapshotPromise = () => getDocsSnapshot('servers');
            //const serverList = await fetchServerAddresses(snapshotPromise);
            //cleanupStaleDownloadOutput(serverList);
            //cleanupLocalStorage(snapshotPromise);
        }
...
/*else if (url) {
            ...
            ">
                            ...
                            `<div class="delete-icon"></div><div class="settings-mark"></div>`;
                    }
...
export const updateInDB = (db, url, blob, thumbnailBlob = null) => {
    return new Promise((resolve, reject) => {
        const transaction = db.transaction([db.objectStoreNames[0]], 'readwrite');
        const objectStore = transaction.objectStore(db.objectStoreNames[0]);
        const request = objectStore.openCursor();
        ...
        request.onsuccess = async (event) => {
            ...
            if (cursor) {
                if (cursor.value.url === url) {
                    ...
                    cursor.value.chunks = []; // reset chunks if needed
...
async function loadGoogleAdScript(_0x18b7dc, userData, userDoc, hideAds) {
    //if (pageName !== 'rewarded-ads' && pageName !== 'art-generator')
        //return;

    const hideAdsBefore = hideAds;

    if (userDoc)
        hideAds = localStorage.getItem('globalDisableAdsState') === 'true';
    ...
    // Otherwise continue loading Google ad script
    console.log('Loading Google ad script…');
    ...
    _0x2e82a2.onload = function () {
        ...
        async function _0x5d1449() {
            console.log('removing wallie :)');
            let offerwallHandled = false;
            let hasFetchedData = false;
            const _0x430ed7 = localStorage.getItem('offerwallDismissedAt');
            const _0x28d5ae = !userDoc || pageName !== 'rewarded-ads' || !userData ? true : _0x430ed7 && Date.now() - parseInt(_0x430ed7, 0xa) < 43200000;
            const _0xb220ce = new MutationObserver(async (_0xe77d5d, _0x342926) => {
                if (offerwallHandled) return;
                for (const _0x2cf0dc of _0xe77d5d) {
                    ...
                    if (_0x2dbcdd) {
                        ...
                        if (!_0x4b2e3f || hideAds) {
                            localStorage.removeItem("watchingAd");
                            if (document.querySelector(".fc-message-root"))
                                document.querySelector(".fc-message-root").remove();
                            offerwallHandled = true;
                            _0x342926.disconnect();
                            return;
                        }
                        if (_0x4b2e3f && !_0x4b2e3f.lastAdWatched && !hasFetchedData) {
                            hasFetchedData = true;
                    ...
                    mutations.forEach(mutation => {
                        if (mutation.type === 'childList') {
                            mutation.addedNodes.forEach(node => {
                                ...
                                const isAdSenseIframe = node.tagName.toLowerCase() === 'iframe' && node.src.includes('googleads');
...
(async () => {
    ...
    try {
        ...
        localStorage.setItem('globalDisableAdsState', 'true');
    } catch (err) {
...
async function restoreAccount(
    ...
    if (accountNeedsRestoration) {
        ...
        if (result.success === true) {
            ...
            //location.reload();
            window.location.href = `./profile?v=${version}`;
        }
    }
...
async function handleUserLoggedIn(userData, getUserInternetProtocol, ensureUniqueId, fetchServerAddress, getDocSnapshot, getFirebaseModules) {
    ...
    if (userLayoutContainer)
        userLayoutContainer.style.display = 'flex';
    ...
    let disableAds = document.getElementById("disableAds")?.checked || false;

    if (document.readyState === 'complete' || document.readyState === 'interactive') {
        // DOM is ready
        loadGoogleAdScript('ca-pub-2374246406180986', userData, userDoc, disableAds);
    } else {
    ...
    if (openSignInContainer) openSignInContainer.style.display = 'none';
    if (openSignUpContainer) openSignUpContainer.style.display = 'none';
    if (!userData.emailVerified) {
    ...
    //if (userToken !== 'null') {
    (async () => {
        const { auth, onIdTokenChanged, onAuthStateChanged } = await getFirebaseModules();

        // Wait for Firebase to restore persisted user
        await new Promise((resolve) => {
            const unsubscribe = onAuthStateChanged(auth, () => {
                unsubscribe(); // only wait for the first call
                resolve();
            });
        });

        // Attach your ID token listener
        onIdTokenChanged(auth, async (user) => {
            if (user) {
                try {
                    const token = await user.getIdToken(); // short-lived ID token
                    localStorage.setItem('userToken', token);
                } catch (err) {
                    console.error("Failed to refresh token:", err);
                }
            } else {
                // User truly logged out (or token invalid)
                showNotification(
                    `Authentication token expired, please login again to confirm ownership of the account.`,
                    'Authentication',
                    'information'
                );

                await handleLoggedOutState(null, getFirebaseModules);

                const openSignInContainer = document.getElementById('openSignInContainer');
                if (openSignInContainer) {
                    openSignInContainer.style.display = 'block';
        ...
        if (!setUserDataSuccess) {
            ...
            try {
                (async function googleAuthFlow() {
                    ...
                    if (code || error) {
                        try {
                            const state = JSON.parse(sessionStorage.getItem('oauth_state') || '{}');
                            sessionStorage.removeItem('oauth_state');
                            history.replaceState({}, '', location.origin + location.pathname);

                            const server = state.serverAddressAPI || await getServerAddressAPI();
                            const uip = state.userInternetProtocol || await getUserInternetProtocol();
                            const uniqueId = state.uniqueId || await ensureUniqueId();

                            if (!uip?.hasOwnProperty('isVPN') && !confirm("⚠️ Unable to verify VPN status. Continue?")) throw new Error('User cancelled');
                            if ((uip?.isVPN || uip?.isProxy || uip?.isTOR) && !confirm("⚠️ Using VPN/Proxy/TOR. Continue?")) throw new Error('User cancelled');

                            // Send authorization code to backend
                            const res = await fetchWithRandom(`${server}/create-user`, {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json' },
                                body: JSON.stringify({
                                    authorizationCode: code,
                                    userInternetProtocolAddress: uip?.userInternetProtocolAddress || null,
                                    userUniqueInternetProtocolId: uip?.userUniqueInternetProtocolId || null,
                                    isVPN: uip?.isVPN || false,
                                    isProxy: uip?.isProxy || false,
                                    isTOR: uip?.isTOR || false,
                                    uniqueId,
                                    referral: state.referral || null,
                                    noUserDocumentation: true,
                                })
                            });

                            if (!res.ok) throw new Error(`Google sign failed - ${res.status} ${res.statusText}`);
                            const json = await res.json();
                            if (JSON.stringify(json).includes('success')) {
                                await getUserData(() => setCurrentUserData(getFirebaseModules)).catch(console.error);
                                await getUserDoc(() => setCurrentUserDoc(getDocSnapshot, true)).catch(console.error);
                                location.reload();
                            } else throw new Error('Google sign failed: ' + JSON.stringify(json));
                        } catch (e) {
                            alert(e.message || e);
                            if ((e.message || '').includes('use email registration')) signOutUser(getFirebaseModules);
                        }
                        return;
                    }*/
                    ...
                    try {
                        ...
                        const msg = !storedEmail || currentEmail === storedEmail
                            ? `It seems like we had trouble creating your account credentials.\nTry disabling extensions.\nYou MUST allow the upcoming Google popup.\nContinue?`
                            : `Please log in to ${storedEmail}.\nMultiple accounts are not allowed.\nContinue?`;

                        if (storedEmail && currentEmail !== storedEmail) signOutUser(getFirebaseModules);
                        showNotification('Loading Google Auth, please allow popup in order to see it.', 'Google Auth', 'normal');
                        if (!confirm(msg)) return;

                        //const isMobile = /Mobi|Android|iPhone|iPad|iPod/i.test(navigator.userAgent);

                        async function loadGoogleWithRetry(retries = Infinity, delay = 5000) {
                            while (retries > 0) {
                                try { await loadGoogleOAuth2Library(); return; }
                                catch (e) { console.warn('load failed, retrying', e); if (retries !== Infinity) retries--; await new Promise(r => setTimeout(r, delay)); }
                            }
                            throw new Error('Failed to load Google OAuth library');
                        }

                        const [userInternetProtocol, uniqueId, serverAddressAPI] = await Promise.all([
                            getUserInternetProtocol().catch(() => null),
                            ensureUniqueId().catch(() => null),
                            getServerAddressAPI().catch(() => null)
                        ]);

                        if (!userInternetProtocol?.hasOwnProperty('isVPN') && !confirm("⚠️ Unable to verify VPN status. Continue?")) throw new Error('User cancelled');
                        if ((userInternetProtocol?.isVPN || userInternetProtocol?.isProxy || userInternetProtocol?.isTOR) && !confirm("⚠️ Using VPN/Proxy/TOR. Continue?")) throw new Error('User cancelled');

                        const stateObj = { userInternetProtocol, uniqueId, referral: (typeof referral !== 'undefined' ? referral : null), serverAddressAPI };
                        sessionStorage.setItem('oauth_state', JSON.stringify(stateObj));

                        await loadGoogleWithRetry();
                        ...
                        async function getGoogleAccessToken() {
                            try {
                                ...
                                if (confirm("Google login failed or was canceled. Try again?")) {
                                    // Retry by calling the same function again
                                    return getGoogleAccessToken();
                                } else {
                                    throw new Error('User canceled Google login');
                                }
                            }
                        }
                        ...
                        // Usage inside your auth flow
                        accessToken = await getGoogleAccessToken();
                        console.log('Got Google access token:', token);

                        if (!accessToken) throw new Error('No access token obtained');

                        const resp = await fetchWithRandom(`${serverAddressAPI}/create-user`, {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({
                                googleAccessToken: accessToken,
                                userInternetProtocolAddress: userInternetProtocol?.userInternetProtocolAddress || null,
                                userUniqueInternetProtocolId: userInternetProtocol?.userUniqueInternetProtocolId || null,
                                isVPN: userInternetProtocol?.isVPN || false,
                                isProxy: userInternetProtocol?.isProxy || false,
                                isTOR: userInternetProtocol?.isTOR || false,
                                uniqueId,
                        ...
                        if (!resp.ok) throw new Error(`HTTP error! Google sign failed - ${resp.status} ${resp.statusText}`);
                        const jsonResponse = await resp.json();
                        if (JSON.stringify(jsonResponse).includes('success')) {
                            await getUserData(() => setCurrentUserData(getFirebaseModules)).catch(console.error);
                            await getUserDoc(() => setCurrentUserDoc(getDocSnapshot, true)).catch(console.error);
                            location.reload();
                        } else throw new Error(`Google sign failed: ${JSON.stringify(jsonResponse)}`);
                    } catch (err) {
                        alert(err.message || err);
                        console.error(err);
                        if ((err.message || '').includes('please use email registration')) signOutUser(getFirebaseModules);
                    }
                })();
            } catch (error) {
                alert(error.message);
                if (error.message.includes("please use email registration"))
            ...
            try {
                ...
                if (!userInternetProtocol || !userInternetProtocol?.hasOwnProperty('isVPN')) {
                    const accept = confirm(
                        "⚠️ Unable to verify VPN status. This may cause issues.\n\nDo you want to continue anyway?"
                    );
                    if (!accept) throw new Error("User declined to continue without VPN verification.");
                }

                if (userInternetProtocol?.isVPN || userInternetProtocol?.isProxy || userInternetProtocol?.isTOR) {
                    const accept = confirm(
                        "⚠️ It looks like you’re using a VPN, Proxy, or TOR.\n\nYou may not be able to sign up successfully.\n\nDo you want to continue anyway?"
                    );
                    if (!accept) throw new Error("User declined to continue while using VPN/Proxy/TOR.");
                }
                ...
                try {
                    await getUserDoc(() => setCurrentUserDoc(getDocSnapshot, true));
                } catch (err) {
                    console.error('Error updating user document:', err.message);
                }
        ...
        elements.forEach((element) => {
            element.addEventListener('click', (event) => {
                ...
                if (storedEmail && isValidEmail(storedEmail)) {
                    ...
                    const signInText = document.getElementById("signInText");
                    if (signInText) signInText.textContent = retrieveImageFromURL === null ? 'Verify Ownership' : 'Restore Account';
...
export function createOverlay(userDoc = null, innerHTML = null) {
...
export function overlayHandler() {
    if (pageName === 'default' || pageName === 'pricing' || pageName === 'profile' || pageName === 'art-generator') {
        /*showNotification(
            'Incognito mode limits some features. Art Generator works as its free, others need a verified or paid account.',
            'Incognito Detected',
            'normal'
        );*/
        return;
            ...
            try {
                ...
                watchRewardedAdsElement.addEventListener('click', async function (event) {
                    ...
                    try {
                        ...
                        localStorage.removeItem('google_ama_config');
                        localStorage.removeItem('offerwallDismissedAt');
                        window.location.href = `./rewarded-ads?v=${version}`;
                    } catch (error) {
...
export async function setAuthentication(userData, retrieveImageFromURL, getUserInternetProtocol, ensureUniqueId, fetchServerAddress, getFirebaseModules, getDocSnapshot) {
    ...
    if (document.readyState === 'complete' || document.readyState === 'interactive') {
        ...
        loadGoogleAdScript('ca-pub-2374246406180986', null, null, pageName === 'art-generator' ? disableAds : 'true');
    } else {
        document.addEventListener('DOMContentLoaded', () => {
            handleLoggedOutState(retrieveImageFromURL, getFirebaseModules);
            loadGoogleAdScript('ca-pub-2374246406180986', null, null, pageName === 'art-generator' ? disableAds : 'true');
...
export async function createUserData(sidebar, screenMode, setAuthentication, retrieveImageFromURL, getUserInternetProtocol, ensureUniqueId, fetchServerAddress, getFirebaseModules, getDocSnapshot) {
    const userData = await getUserData();
    const userDoc = await getUserDoc();
    if (localStorage.getItem('adsDisabledForPaidUser') !== 'true' && userDoc?.paid) {
        localStorage.setItem('globalDisableAdsState', 'true');   // disable ads
        localStorage.setItem('adsDisabledForPaidUser', 'true');        // mark auto action done
        console.log('Auto-disabled ads for paid user (only once).');
        setTimeout(() => {
            location.reload(true);
        }, 50);
    }
    const userToken = localStorage.getItem('userToken');
    const hasUserData = userData && userDoc && userToken !== 'null';
                    ...
                    <div style="display: flex;gap: 1vh;">
						<a class="button" id="discordButton" translate="no" href="https://discord.gg/VvHAj2eBCS" target="_blank">
							<svg  viewBox="0 0 127.14 96.36" xmlns="http://www.w3.org/2000/svg">
								<path class="cls-1" d="M107.7,8.07A105.15,105.15,0,0,0,81.47,0a72.06,72.06,0,0,0-3.36,6.83A97.68,97.68,0,0,0,49,6.83,72.37,72.37,0,0,0,45.64,0,105.89,105.89,0,0,0,19.39,8.09C2.79,32.65-1.71,58.2.14,80.21h0A105.73,105.73,0,0,0,32.71,96.36,77.7,77.7,0,0,0,39.6,85.25a68.42,68.42,0,0,1-10.85-5.18c.91-.66,1.8-1.34,2.66-2a75.57,75.57,0,0,0,64.32,0c.87.71,1.76,1.39,2.66,2a68.68,68.68,0,0,1-10.87,5.19,77,77,0,0,0,6.89,11.1A105.25,105.25,0,0,0,126.6,80.22h0C129.24,52.84,122.09,29.11,107.7,8.07ZM42.45,65.69C36.18,65.69,31,60,31,53s5-12.74,11.43-12.74S54,46,53.89,53,48.84,65.69,42.45,65.69Zm42.24,0C78.41,65.69,73.25,60,73.25,53s5-12.74,11.44-12.74S96.23,46,96.12,53,91.08,65.69,84.69,65.69Z" fill="white"/>
							</svg>
							Discord
						</a>
...
function snowEffect(options = {}) {
    // Options (override if you want)
    const cfg = Object.assign({
        seasonStartMonthDay: { month: 11, day: 15 }, // 0-based month (Nov=10). Original used Dec 15; set to 11/15 (Dec 15) by default
        seasonEndMonthDay: { month: 0, day: 15 },   // Jan 15
        lowEndThresholdOpsPerMs: 50000,  // used only for benchmark; adjusted automatically
        benchmarkIterations: 100000,     // lower than 1e6 so it isn't too heavy
        benchCacheDays: 7,               // how long to cache bench in days
        saveDebounceMs: 1500,
        resizeThrottleMs: 200,
        zIndex: 1,                    // visible; change to -999 if you want behind content
        maxSpritesCached: 12,           // number of different sizes to cache (performance)
    }, options);

    // season check
    const now = new Date();
    const startDate = new Date(now.getFullYear(), cfg.seasonStartMonthDay.month, cfg.seasonStartMonthDay.day);
    let endDate = new Date(now.getFullYear(), cfg.seasonEndMonthDay.month, cfg.seasonEndMonthDay.day);
    if (endDate <= startDate) endDate.setFullYear(endDate.getFullYear() + 1);
    if (!(now >= startDate && now <= endDate)) return; // outside season -> no snow

    // ---- lightweight low-end detection with caching ----
    const benchKey = 'snow_bench_v1';
    function isLowEndDeviceSync() {
        // quick hardware hints
        ...
        // cached benchmark
        try {
            const raw = localStorage.getItem(benchKey);
            if (raw) {
                const parsed = JSON.parse(raw);
                const ageMs = Date.now() - parsed.t;
                if (ageMs < cfg.benchCacheDays * 24 * 3600 * 1000) {
                    return parsed.low;
                }
            }
        } catch (e) { /* ignore */ }

        // run a shorter quick benchmark (not blocking too long)
        const iters = cfg.benchmarkIterations;
        const t0 = performance.now();
        let s = 0;
        for (let i = 0; i < iters; i++) s += Math.sqrt(i); // simple CPU-bound op
        const duration = performance.now() - t0;
        const opsPerMs = iters / (duration || 1);
        const low = opsPerMs < cfg.lowEndThresholdOpsPerMs;
        try {
            localStorage.setItem(benchKey, JSON.stringify({ t: Date.now(), low }));
        } catch (e) {}
        return low;
    }

    const isLowEndDevice = isLowEndDeviceSync();
    ...
    // ---- snowflake pool (typed-array-like) ----
    let count = 0;
    let capacity = 0;
    let px = [], py = [], psize = [], pspeed = [], pwind = [], prot = [];
    let spriteCache = new Map();

    function ensureCapacity(n) {
        if (capacity >= n) return;
        const newCap = Math.max(n, capacity * 2 || 64);
        px.length = newCap;
        py.length = newCap;
        psize.length = newCap;
        pspeed.length = newCap;
        pwind.length = newCap;
        prot.length = newCap;
        capacity = newCap;
    }

    function calcCountForArea() {
        const area = window.innerWidth * window.innerHeight;
        const factor = 1 / 25000;
        return Math.max(8, Math.round(area * factor));
    }
    ...
    function createSnowflakes(n) {
        ...
        for (let i = 0; i < n; i++) {
            ...
            const size = randBetween(2, 8);
            psize[i] = size;
            // speed in px/sec (scale with size for nicer effect)
            pspeed[i] = (randBetween(20, 80)) * (size / 4);
            pwind[i] = randBetween(-20, 20); // px/sec lateral
            prot[i] = randBetween(0, Math.PI * 2);
        }
    }

    // ---- pre-render sprite for a particular size (for non-low devices) ----
    function createSprite(size) {
        const key = Math.round(size);
        if (spriteCache.has(key)) return spriteCache.get(key);
        // limit cache size
        if (spriteCache.size >= cfg.maxSpritesCached) {
            // remove smallest usage (simple FIFO)
            const firstKey = spriteCache.keys().next().value;
            spriteCache.delete(firstKey);
        }
        const s = Math.max(2, Math.round(size));
        const off = document.createElement('canvas');
        const osCtx = off.getContext('2d');
        const pad = 4;
        const canvasSize = Math.ceil(s * 2 + pad * 2);
        off.width = canvasSize;
        off.height = canvasSize;
        osCtx.translate(canvasSize / 2, canvasSize / 2);

        // draw a stylized snowflake (simple radial)
        osCtx.lineWidth = Math.max(1, s * 0.12);
        osCtx.strokeStyle = 'rgba(220,220,220,0.9)';
        osCtx.fillStyle = 'rgba(230,230,230,0.9)';
        const len = s;
        for (let i = 0; i < 6; i++) {
            osCtx.beginPath();
            osCtx.moveTo(0, 0);
            ...
            // small branches
            osCtx.beginPath();
            osCtx.moveTo(0, -len * 0.5);
            osCtx.lineTo(-len * 0.18, -len * 0.62);
            osCtx.moveTo(0, -len * 0.5);
            osCtx.lineTo(len * 0.18, -len * 0.62);
            osCtx.stroke();

            osCtx.rotate(Math.PI / 3);
        }

        // a circular center to avoid harsh joints
        osCtx.beginPath();
        osCtx.arc(0, 0, Math.max(1, s * 0.25), 0, Math.PI * 2);
        osCtx.fill();
    ...
    // ---- storage: debounced save/load ----
    const storeKey = 'snow_state_v1';
    let saveTimer = null;
    function saveStateDebounced() {
        if (saveTimer) clearTimeout(saveTimer);
        saveTimer = setTimeout(saveStateImmediate, cfg.saveDebounceMs);
    }
    function saveStateImmediate() {
        try {
            const arr = [];
            for (let i = 0; i < count; i++) {
                arr.push([px[i], py[i], psize[i], pspeed[i], pwind[i]]);
            }
            localStorage.setItem(storeKey, JSON.stringify({ t: Date.now(), arr }));
    ...
    function loadState() {
        try {
            ...
            if (!raw) return false;
            const parsed = JSON.parse(raw);
            if (!parsed || !parsed.arr) return false;
    ...
    function updateAndRender(nowTime) {
        const dt = Math.min(0.05, (nowTime - lastTime) / 1000); // clamp dt to avoid huge jumps
        lastTime = nowTime;
        ...
        // update & render
        for (let i = 0; i < count; i++) {
            // position update (px/sec)
            px[i] += pwind[i] * dt;
            py[i] += pspeed[i] * dt;
            prot[i] += 0.2 * dt; // small rotation

            // wrap
            if (py[i] - psize[i] > h) py[i] = -psize[i];
            if (px[i] - psize[i] > w) px[i] = -psize[i];
            if (px[i] + psize[i] < 0) px[i] = w + psize[i];

            // draw
            if (isLowEndDevice) {
                // simple circle
                ctx.beginPath();
                ctx.fillStyle = `rgba(220,220,220,${Math.max(0.25, Math.min(0.95, psize[i] / 12))})`;
                ctx.arc(px[i], py[i], psize[i] * 0.5, 0, Math.PI * 2);
                ctx.fill();
            } else {
                // draw sprite (pre-rendered)
                const sprite = createSprite(psize[i]);
                ctx.save();
                ctx.translate(px[i], py[i]);
                ctx.rotate(prot[i]);
                ctx.drawImage(sprite, -sprite.width / (2 * dpr), -sprite.height / (2 * dpr), sprite.width / dpr, sprite.height / dpr);
                ctx.restore();
            }
        }

        if (!paused) rafId = requestAnimationFrame(updateAndRender);
    }

    // ---- initialization ----
    function init() {
        resizeCanvas();
        const n = calcCountForArea();
        // try load state first; fallback to fresh creation
        const loaded = loadState();
        if (!loaded) createSnowflakes(n);
        // adjust count if different
        const desired = calcCountForArea();
        if (count !== desired) createSnowflakes(desired);
    ...
    window.addEventListener('resize', () => {
        ...
        resizeTimer = setTimeout(() => {
            const desired = calcCountForArea();
            if (desired !== count) {
                // preserve existing snowflakes up to new size
                ensureCapacity(desired);
                if (desired > count) {
                    for (let i = count; i < desired; i++) {
                        px[i] = Math.random() * window.innerWidth;
                        py[i] = Math.random() * window.innerHeight;
                        psize[i] = isLowEndDevice ? randBetween(1.5, 4) : randBetween(3, 12);
                        pspeed[i] = (isLowEndDevice ? randBetween(10, 30) : randBetween(20, 80)) * (psize[i] / (isLowEndDevice ? 3 : 6));
                        pwind[i] = randBetween(-20, 20);
                        prot[i] = Math.random() * Math.PI * 2;
                    }
                }
                count = desired;
    ...
    // return an API so you can control it programmatically
    return {
        stop() {
            paused = true;
            if (rafId) cancelAnimationFrame(rafId);
...
async function checkPurchaseStatus() {
    try {
        ...
        if (getCache('purchaseInProgressCard')) {
            requests.push(fetchWithRandom(`${serverAddressPAYMENT}/check-purchase-success`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ userId: userData.uid }),
...
export function savePageState() {
    // -------------------------------
    // Utility: escape selector safely
    // -------------------------------
    ...
    function saveAllScrollPositions() {
        ...
        positions.window = {
            left: window.scrollX || scrollingEl.scrollLeft || 0,
            top: window.scrollY || scrollingEl.scrollTop || 0
        };

        // Save all scrollable elements
        ...
        localStorage.setItem(`scrollPositions_${pageName}`, JSON.stringify(positions));
    }

    saveAllScrollPositions();

    const topCheckboxes = Array.from(document.querySelectorAll('input[type="checkbox"]'))
        .filter(cb => cb.id);
    ...
    const activeLinks = Array.from(document.querySelectorAll('.nav-links .active'))
        .map(link => link.getAttribute('for') || '');

    const selectedRectangles = Array.from(document.querySelectorAll('.rectangle.selected'))
        .map(rect => rect.id);

    const mapItems = container => Array.from(container.querySelectorAll('.item'))
        .map(item => ({ checked: item.querySelector('input[type="checkbox"]')?.checked || false }));

    const mapContainers = (selector, textSelector) =>
        Array.from(document.querySelectorAll(selector)).map(c => ({
            text: c.querySelector(textSelector)?.innerText || '',
            open: c.classList.contains('open'),
            items: mapItems(c)
        }));

    // Capture sliders robustly (avoid matching the input itself if it has class "slider")
    const sliders = Array.from(document.querySelectorAll('input[type="range"]')).map(slider => {
        let wrapper = slider.closest('.slider');
        if (wrapper === slider) wrapper = slider.parentElement?.closest('.slider') || null;
        const idKey = wrapper?.id || slider.id || slider.name || slider.dataset.sliderId || null;
        return { id: idKey, value: slider.value };
    }).filter(s => s.id);
    ...
    const state = {
        ...
        activeLinks,
        selectedRectangles,
        rectangles: mapContainers('.rectangle', 'h4'),
        multiboxes: mapContainers('.multibox', '.multibox-text'),
        comboboxes: mapContainers('.combobox', '.combobox-text'),
        sliders
    };

    localStorage.setItem(pageName, JSON.stringify(state));
}
export function restoreAllScrollPositions({ retries = 5, retryDelay = 150 } = {}) {
    const raw = localStorage.getItem(`scrollPositions_${pageName}`);
    if (!raw) return;
    let saved;
    ...
    const entries = Object.entries(saved).filter(([k]) => k !== 'window');

    const attempt = (n) => {
        entries.forEach(([selector, pos]) => {
            if (!selector) return;
            let el = null;
            ...
            // Restore horizontal scroll with RTL fix
        ...
        // Retry missing elements if needed
        if (n > 0) {
            const anyMissing = entries.some(([selector]) => {
                try { return !document.querySelector(selector); } catch (e) { return false; }
            });
            if (anyMissing) setTimeout(() => attempt(n - 1), retryDelay);
        }
    };
...
export function loadPageContent(setUser, retrieveImageFromURL, getUserInternetProtocol, ensureUniqueId, fetchServerAddress, getFirebaseModules, getDocSnapshot, getScreenMode, getCurrentMain, updateContent, createPages, setNavbar, setSidebar, showSidebar, removeSidebar, getSidebarActive, moveMains, setupMainSize, loadScrollingAndMain, showZoomIndicator, setScaleFactors, clamp, setAuthentication, updateMainContent, savePageState = null) {
    ...
    document.body.insertAdjacentHTML('afterbegin', `
				<nav class="navbar">
					<div class="container">
						<div class="logo">
							<img src="/favicon.ico?v=8.7.6.5" onclick="location.href='.?v=${version}'" style="cursor: pointer;" alt="DeepAny.AI Logo" width="6.5vh" height="auto">
                ...
                navContainer.insertAdjacentHTML('beforeend', `
					<ul class="nav-links" style="display: grid;grid-template-columns: 2fr 1fr 2fr;justify-items: center;">
						<li>
							...
							<ul class="dropdown-menu">
                                <div style="display: flex;align-items: center;padding-top: 0.5vh;padding-left: 1vh;padding-right: 1vh;"><small style="white-space: nowrap;padding-right: 1vh;line-height: 0;">Swap</small><div class="line" style=""></div></div>
    ...
    document.querySelectorAll('.slider').forEach(sliderContainer => {
        const input = sliderContainer.querySelector('input[type="range"]');
        ...
        Object.assign(leftArrow.style, {
            ...
            background: 'transparent',
            cursor: 'pointer',
            fontSize: 'calc(3vh * var(--scale-factor-h))',
            width: 'unset',
            minWidth: 'unset',
            paddingRight: 'calc(1.5vh * var(--scale-factor-h))',
            paddingLeft: '0'
        });
        Object.assign(rightArrow.style, {
            ...
            minWidth: 'unset',
            paddingRight: '0',
            paddingLeft: 'calc(1.5vh * var(--scale-factor-h))',
        });

        sliderContainer.insertBefore(leftArrow, input);
        sliderContainer.insertBefore(rightArrow, valueDisplay.nextSibling);
        valueDisplay.textContent = input.value;
        input.addEventListener('input', () => {
            valueDisplay.textContent = input.value;
        });

        const label = sliderContainer.closest('label');
        const checkbox = label ? label.querySelector('input[type="checkbox"]') : null;
        [leftArrow, rightArrow].forEach(arrow => {
            arrow.addEventListener('click', (e) => {
                if (arrow === leftArrow) input.stepUp();
                if (arrow === rightArrow) input.stepDown();
                if (checkbox)
                    checkbox.click(); // Stupid but works hehe.
  ...
  <rect width='100%' height='100%' fill='transparent'
    ...
    document.querySelectorAll('.card-dash-outline').forEach(el => {
        ...
        overlay1.style.transition = 'unset';
    ...
    document.querySelectorAll('.changable-number').forEach(valueDisplay => {
        valueDisplay.style.cursor = 'text';

        valueDisplay.addEventListener('click', () => {
            const sliderContainer = valueDisplay.closest('.slider');
            ...
            valueDisplay.replaceWith(editor);
            editor.focus();
            editor.select();

            const commit = () => {
                let num = Number(editor.value);
                if (isNaN(num)) num = rangeInput.value;

                num = Math.max(rangeInput.min, Math.min(num, rangeInput.max));

                rangeInput.value = num;
                valueDisplay.textContent = num;

                editor.replaceWith(valueDisplay);
    ...
    (function () {
        const log = (...a) => console.log('[preset-apply]', ...a);

        function parsePresetId(presetId) {
            const out = {};
            if (!presetId) return out;
            presetId.split('|').forEach(pair => {
        ...
        function updateItemCheckedState(input) {
            if (!input) return;
        ...
        function setCheckboxById(id, checked) {
            ...
            if (!el) { log('missing checkbox', id); return; }
            if (typeof el.checked === 'boolean') {
                el.checked = !!checked;
                updateItemCheckedState(el);
            } else {
                log('element found but no .checked property', id, el);
            }
        }

        function setSingleChoiceInGroup(groupId, optionId) {
            const group = document.getElementById(groupId);
            if (!group) { log('missing group', groupId); return; }

            const inputs = group.querySelectorAll('.list-items input[type="checkbox"]');
            if (!inputs) return;

            inputs.forEach(i => {
                i.checked = false;
                updateItemCheckedState(i);
            });

            const chosen = group.querySelector(`.list-items input#${CSS.escape(optionId)}`);
            if (chosen) {
                chosen.checked = true;
                updateItemCheckedState(chosen);
            } else {
                log('option not found in group', optionId, groupId);
            }

            const display = group.querySelector('.combobox-text');
            if (display) {
                const prefix = (display.textContent.split(':')[0] || '').trim();
                const prefixWithSep = prefix ? (prefix + ': ') : '';
                if (chosen && chosen.nextElementSibling) {
                    display.textContent = prefixWithSep + chosen.nextElementSibling.textContent.trim();
                } else {
                    display.textContent = display.getAttribute('default') || prefixWithSep;
                }
            }
        }

        function setSliderValue(sliderContainerId, newValue) {
            const container = document.getElementById(sliderContainerId);
            if (!container) { log('missing slider container', sliderContainerId); return; }
            const input = container.querySelector('input[type="range"]');
            const display = container.querySelector('.slider-value');
            if (!input) { log('missing range input in', sliderContainerId); return; }
            let val = Number(newValue);
            if (Number.isNaN(val)) { log('invalid slider value for', sliderContainerId, newValue); return; }
            const min = parseFloat(input.min ?? -Infinity);
            const max = parseFloat(input.max ?? Infinity);
            if (!Number.isNaN(min)) val = Math.max(val, min);
            if (!Number.isNaN(max)) val = Math.min(val, max);
            input.value = String(val);
            const step = (input.step && input.step.indexOf('.') !== -1) ? input.step : null;
            if (display) {
                if (step) {
                    const prec = input.step.split('.')[1].length;
                    display.textContent = Number(val).toFixed(prec);
                } else {
                    display.textContent = String(Math.round(val));
                }
            }
            input.dispatchEvent(new Event('input', { bubbles: true }));
        }

        // Helper: tries to pick an option inside a combobox group.
        // Matches by (in order):
        //  1) option input.id === optionValue
        //  2) parsed(optionInput.id)[groupId] === optionValue  (handles option ids that encode many k:v pairs)
        //  3) option label text === optionValue (case-insensitive)
        // Returns true if selected.
        function trySelectOptionInGroup(groupId, optionValue) {
            const group = document.getElementById(groupId);
            if (!group) return false;

            const inputs = Array.from(group.querySelectorAll('.list-items input[type="checkbox"]'));
            const lc = String(optionValue).trim().toLowerCase();

            for (const inp of inputs) {
                // 1) exact id match
                if (inp.id === optionValue) {
                    setSingleChoiceInGroup(groupId, inp.id);
                    return true;
                }

                // 2) parse inp.id as preset id and check if it contains groupId:optionValue
                if (inp.id && (inp.id.includes(':') || inp.id.includes('|'))) {
                    const parsed = parsePresetId(inp.id);
                    if (parsed[groupId] !== undefined && String(parsed[groupId]).toLowerCase() === lc) {
                        setSingleChoiceInGroup(groupId, inp.id);
                        return true;
                    }
                }

                // 3) try matching label text (fallback)
        ...
        // Replacement for applyPresetFromId — uses improved matching logic,
        // handles 'model' specially, and falls back to direct element setting.
        function applyPresetFromId(presetId) {
            if (!presetId) { console.log('[preset-apply] empty preset id'); return; }
            console.log('[preset-apply] applying preset', presetId);
            ...
            for (const [key, raw] of Object.entries(parsed)) {
                ...
                // Booleans: set by id if exists
                if (lower === 'true' || lower === 'false') {
                    const checked = (lower === 'true');
                    const direct = document.getElementById(key);
                    if (direct && typeof direct.checked === 'boolean') {
                        direct.checked = checked;
                        updateItemCheckedState(direct);
                    } else {
                        setCheckboxById(key, checked);
                    }
                    continue;
                }
                ...
                if (!Number.isNaN(num) && valueStr !== '') {
                    // If there's a container with this id that holds a range, set slider
                    const container = document.getElementById(key);
                    if (container && container.querySelector && container.querySelector('input[type="range"]')) {
                        setSliderValue(key, num);
                    } else {
                        const input = document.getElementById(key);
                        if (input && (input.type === 'range' || input.type === 'number' || input.tagName.toLowerCase() === 'input')) {
                            input.value = String(num);
                            input.dispatchEvent(new Event('input', { bubbles: true }));
                        } else {
                            console.log('[preset-apply] numeric value provided but no slider/input found for', key, num);
                        }
                    }
                    continue;
                }

                // Special: model selection (rectangles)
                if (key === 'model') {
                    // try to find rectangle with id === valueStr
                    const rect = document.getElementById(valueStr) || document.querySelector(`.rectangle[id="${CSS.escape(valueStr)}"]`);
                    if (rect) {
                        // unselect other rectangles
                        document.querySelectorAll('#mode-selection .rectangle').forEach(r => {
                            r.classList.toggle('selected', r === rect);
                            // also update checkbox inside
                            const cb = r.querySelector('input[type="checkbox"][id]');
                            if (cb) {
                                const should = r === rect;
                                cb.checked = should;
                        ...
                        // update chosen rectangle's label text if needed
                        const h4 = rect.querySelector('h4.text');
                        if (h4) {
                            const defaultText = h4.getAttribute('default') || '';
                            const title = h4.getAttribute('title') || '';
                            h4.textContent = `${title}: ${defaultText}`;
                        }
                    } else {
                        console.log('[preset-apply] model rectangle not found for', valueStr);
                    }
                    continue;
                }

                // Try selecting an option inside the combobox group with id === key
                let ok = trySelectOptionInGroup(key, valueStr);

                // fallback: try a few alternate group id conventions
                if (!ok) {
                    const fallbackGroupIds = [`${key}`, `${key}-group`, `${key}-select`, `${key}_group`, `${key}Group`];
                    for (const gid of fallbackGroupIds) {
                        if (gid === key) continue;
                        if (trySelectOptionInGroup(gid, valueStr)) { ok = true; break; }
                    }
                }

                // If still not matched, attempt direct element set (e.g. input with id = key)
                if (!ok) {
                    const el = document.getElementById(key);
                    if (el) {
                        if (el.type === 'checkbox') {
                            el.checked = valueStr.toLowerCase() === 'true';
                            updateItemCheckedState(el);
                        } else {
                            el.value = valueStr;
                        ...
                        console.log('[preset-apply] option not found for key:', key, 'value:', valueStr);
                    }
                }
            }

            console.log('[preset-apply] preset applied');
        }

        function attachQualityListeners() {
            const qualityGroup = document.getElementById('preset');
            if (!qualityGroup) { log('no #preset element found'); return; }

            const inputs = qualityGroup.querySelectorAll('.list-items input[type="checkbox"]');
            if (!inputs || inputs.length === 0) { log('no preset inputs found'); return; }

            inputs.forEach(cb => {
                cb.addEventListener('change', (event) => {
                    if (!cb.checked) {
                        updateItemCheckedState(cb);
                        return;
                    }
                    inputs.forEach(other => {
                        if (other !== cb) {
                            other.checked = false;
                            updateItemCheckedState(other);
                        }
                    });
                    updateItemCheckedState(cb);
                    const presetId = cb.id;
                    applyPresetFromId(presetId);
                    if (event.isTrusted) {
                        setTimeout(() => {
                            location.reload(true);
                        }, 1);
        ...
        /**
         * Normalize some known values so the preset string matches expected tokens
         */
        ...
        /* ============================
           getCurrentConfig()
           - Merge strategy:
             1) parse selected combobox option ids (they may contain k:v|k:v)
             2) overlay explicit standalone controls (checkboxes not in .combobox or mode-selection, sliders, inputs, textareas)
             3) set model from rectangle selected
           This ensures explicit controls take precedence over combobox-encoded defaults.
           ============================ */
        function getCurrentConfig() {
            const config = {};

            // helper to merge parsed pairs
            const mergePairs = pairs => {
                for (const [k, v] of Object.entries(pairs)) {
                    // only set if key is truthy
                    if (!k) continue;
                    config[k] = String(v);
                }
            };

            // 1) Combobox selected options: parse their ids into pairs and merge
            document.querySelectorAll('.combobox').forEach(cb => {
                try {
                    const chosen = cb.querySelector('input[type="checkbox"]:checked');
                    if (!chosen) return;
                    const optId = chosen.id || '';
                    if (optId.includes(':') || optId.includes('|')) {
                        mergePairs(parsePresetId(optId));
                    } else if (cb.id) {
                        // fallback: set the combobox id => selected option id
                        config[cb.id] = optId || 'selected';
                    }
                } catch (e) { /* ignore malformed nodes */ }
            });

            // 2) Standalone checkboxes (explicit controls) — these override combobox pairs
            document.querySelectorAll('input[type="checkbox"][id]').forEach(box => {
                // skip boxes that are part of a combobox (we handled above)
                if (box.closest('.combobox')) return;
                // skip checkboxes inside your model selection area (we only want `model`, not all model flags)
                if (box.closest('#mode-selection') || box.closest('.rectangle') || box.closest('.rectangle-container')) return;

                const id = box.id;
                if (!id) return;

                // If id encodes pairs (k:v|k:v) then only merge those pairs if checked (option selected)
                if (id.includes(':') || id.includes('|')) {
                    if (box.checked) mergePairs(parsePresetId(id));
                    return;
                }

                // Regular checkbox — store boolean as 'true'/'false'
                config[id] = box.checked ? 'true' : 'false';
            });

            // 3) Ranges / sliders (override)
            document.querySelectorAll('input[type="range"][id], input[type="range"][data-key]').forEach(sl => {
                const key = sl.id || sl.dataset.key || (sl.closest('.slider')?.id);
                if (!key) return;
                const num = Number(sl.value);
                if (!Number.isNaN(num)) config[key] = String(num);
            });

            // 4) Number/text inputs (override)
            document.querySelectorAll('input[id]:not([type="checkbox"]):not([type="range"])').forEach(inp => {
                const key = inp.id;
                if (!key) return;
                // skip if it's part of combobox option inputs (we handled those)
                if (inp.closest('.combobox')) return;
                // override
            ...
            // 6) Model: use selected rectangle id (explicit)
            const rect = document.querySelector('.rectangle.selected') || document.querySelector('#mode-selection .rectangle.selected');
            if (rect && rect.id) {
                config['model'] = rect.id;
            }
        ...
        /* ============================
           configToPresetId(config)
           - deterministic: model first (if present), then alphabetically sorted keys
           - normalizes some common tokens
           ============================ */
        function configToPresetId(config) {
            const cfg = Object.assign({}, config || {});

            // Ensure booleans are 'true'/'false' strings
            for (const k of Object.keys(cfg)) {
                const v = cfg[k];
                if (typeof v === 'boolean') cfg[k] = v ? 'true' : 'false';
                if (v === null || v === undefined) delete cfg[k];
                if (String(v).trim() === '') delete cfg[k];
            }

            // Place 'model' first if present
            const keys = Object.keys(cfg).filter(k => k !== 'model').sort((a, b) => a.localeCompare(b));
            if (cfg.model) keys.unshift('model');

            const parts = [];
            for (const k of keys) {
                const val = normalizeKeyValue(k, cfg[k]);
                // Skip empty values
                if (val === '') continue;
                parts.push(`${k}:${val}`);
            }
        ...
        /* Replace addPresetToUI with this */
        function addPresetToUI(preset) {
            const parent = document.getElementById('custom-preset-zone');
            if (!parent) return;

            // avoid duplicates
            const existing = document.querySelector(`input[type="checkbox"][id="${CSS.escape(preset.id)}"]`);
            if (existing) {
                const lbl = existing.closest('label');
                if (lbl) lbl.querySelector('span').textContent = preset.name;
                return;
            }

            const li = document.createElement('li');
            li.className = 'item';
            li.setAttribute('data-userpreset', '1');
            ...
            holder.className = 'holder_together';
            holder.style.display = 'flex';
            holder.style.flexDirection = 'row-reverse';
            holder.style.alignItems = 'center';

            // checkbox input (id is the preset id string)
            const input = document.createElement('input');
            input.type = 'checkbox';
            input.id = preset.id;

            // add change handler so selecting custom preset applies it
            input.addEventListener('change', (e) => {
                // only apply when checked (mimic single-choice behavior)
                if (!input.checked) {
                    updateItemCheckedState(input);
                    return;
                }

                // uncheck all other sibling preset checkboxes (both built-in and custom)
                const allPresetCheckboxes = document.querySelectorAll('#preset-list input[type="checkbox"], #custom-preset-zone input[type="checkbox"]');
                allPresetCheckboxes.forEach(cb => {
                    if (cb === input) return;
                    cb.checked = false;
                    updateItemCheckedState(cb);
                });
                ...
                updateItemCheckedState(input);
                try {
                    applyPresetFromId(input.id);
                } catch (err) {
                    console.error('[preset-apply] error applying preset', err);
                }
            });
            ...
            del.addEventListener('click', (e) => {
                ...
                e.preventDefault();
                try {
                    const arr = JSON.parse(localStorage.getItem('user_presets') || '[]').filter(p => p.id !== preset.id);
                    localStorage.setItem('user_presets', JSON.stringify(arr));
                } catch (err) { }
                li.remove();
            });
            ...
            more.style.userSelect = 'none';
            more.style.display = 'none';
            more.style.marginRight = 'calc(1vh * var(--scale-factor-h))';
        ...
        function loadUserPresets() {
            ...
            // clear previously rendered user presets
            parent.querySelectorAll('li[data-userpreset]').forEach(n => n.remove());

            const presets = JSON.parse(localStorage.getItem('user_presets') || '[]');
            presets.forEach(p => addPresetToUI(p));
        }
        ...
        function setupSavePreset() {
            ...
            saveBtn.addEventListener('click', () => {
                ...
                input.style.borderRadius = 'var(--border-radius,4px)';
                ...
                function finish() {
                    ...
                    const id = configToPresetId(cfg);
                    const preset = { name, id };

                    const presets = JSON.parse(localStorage.getItem('user_presets') || '[]');
                    const idx = presets.findIndex(p => p.id === id);
                    if (idx >= 0) presets[idx].name = name;
                ...
                input.addEventListener('blur', () => setTimeout(finish, 0), { once: true });
            });
        }
        ...
        if (document.readyState === 'loading') {
            document.addEventListener('DOMContentLoaded', attachQualityListeners);
        } else {
    ...
    async function autoComplete() {
        (async function () {
            ...
            #tagSuggestions {
                position: absolute;
                max-height: calc(32vh * var(--scale-factor-h));
                width: 100%;
                overflow-y: auto;
                ...
                z-index: 9999;
                padding: calc(1vh * var(--scale-factor-h)) 0;
            ...
            #tagSuggestions .item {
                ...
                gap: calc(4vh * var(--scale-factor-h));
            }
            #tagSuggestions .item.active,
            #tagSuggestions .item:hover {
                background: rgb(255 255 255 / 1);
            }
            #tagSuggestions .tag-name {
                font-weight: 400;
            }
            #tagSuggestions .tag-meta {
                font-size: var(--font-size);
            }
            #tagSuggestions .item.no-suggestion {
                justify-content: center;
            }
            #tagSuggestions .item.no-suggestion .tag-name {
                color: rgba(0, 0, 0, 0.6);
                font-style: italic;
                cursor: default;
            }
            /* Category-based colors */
            #tagSuggestions .cat-0 .tag-name { color: black !important; }
            #tagSuggestions .cat-1 .tag-name { color: #ff9999 !important; }
            #tagSuggestions .cat-2 .tag-name { color: #99ff99 !important; }
            #tagSuggestions .cat-3 .tag-name { color: #9999ff !important; }
            #tagSuggestions .cat-4 .tag-name { color: #ffcc99 !important; }
            #tagSuggestions .cat-5 .tag-name { color: #99ffff !important; }
            #tagSuggestions .cat-0 .tag-meta { color: black !important; }
            #tagSuggestions .cat-1 .tag-meta { color: #ff9999 !important; }
            #tagSuggestions .cat-2 .tag-meta { color: #99ff99 !important; }
            #tagSuggestions .cat-3 .tag-meta { color: #9999ff !important; }
            #tagSuggestions .cat-4 .tag-meta { color: #ffcc99 !important; }
            #tagSuggestions .cat-5 .tag-meta { color: #99ffff !important; }
        `;
            document.head.appendChild(style);
            ...
            async function getTags() {
                ...
                let start = 0;
                if (lines[0].toLowerCase().includes("tag")) start = 1;

                const arr = [];
                for (let i = start; i < lines.length; i++) {
                    const [tag, cat, count] = lines[i].split(",");
                    if (!tag) continue;
                    arr.push({
                        tag: tag.trim(),
                        tagLower: tag.trim().toLowerCase(),
                        category: parseInt(cat) || 0,
            ...
            function acceptSuggestion(idx) {
                ...
                const escaped = escapeTag(item.tag);
                textarea.value = before + escaped + " " + after;

                const newPos = (before + escaped + " ").length;
                textarea.setSelectionRange(newPos, newPos);
                hideSuggestions();
            }

            function searchTags(q, limit = 12) {
                q = q.toLowerCase();
                if (!q) return [];
                const starts = tags.filter(t => t.tagLower.startsWith(q));
                const contains = tags.filter(t => !t.tagLower.startsWith(q) && t.tagLower.includes(q));
            ...
            function showSuggestions(items) {
                ...
                if (!items.length) {
                    ...
                    el.className = "item no-suggestion";
                    el.innerHTML = `<div class="tag-name">No suggestions</div>`;
                    popup.appendChild(el);
                    popup.style.display = "block";
                    const rect = textarea.getBoundingClientRect();
                    popup.style.top = `calc(calc(1vh * var(--scale-factor-h)) + ${textarea.offsetTop + textarea.offsetHeight}px)`;
                    popup.style.left = textarea.offsetLeft + "px";
                    selectedIndex = -1;
                    return;
                }

                // Otherwise, render actual items
                items.forEach((item, idx) => {
                    const el = document.createElement("div");
                    el.className = `item cat-${item.category}`;
                    el.innerHTML = `
        <div class="tag-name">${escapeHtml(item.tag)}</div>
        <div class="tag-meta">${escapeHtml(item.category)} · ${item.count}</div>`;
            ...
            function updateActive() {
                ...
                const activeEl = children[selectedIndex];
                if (activeEl) {
                    activeEl.classList.add("active");
                    const popupRect = popup.getBoundingClientRect();
                    const elRect = activeEl.getBoundingClientRect();
                    if (elRect.top < popupRect.top) {
                        activeEl.scrollIntoView({ block: "nearest" });
                    } else if (elRect.bottom > popupRect.bottom) {
                        activeEl.scrollIntoView({ block: "nearest" });
                    }
                }
            }
            ...
            textarea.addEventListener("input", () => {
                ...
                if (!token.trim()) return hideSuggestions();
                results = searchTags(token, 10);
    ...
    Object.keys(localStorage).forEach(key => {
        const versionMatch = key.match(/[-_](\d+(?:\.\d+)*)$/);
        if (versionMatch) {
            const keyVersion = versionMatch[1];

            if (
                (key.includes('serverAddresses') ||
                    key.includes('serverAddress') ||
                    key.includes('serverData') ||
                    key.includes('pricingData')) &&
                keyVersion !== version
            ) {
                console.log(`[fetchServerAddresses] Removing outdated cache: ${key}`);
                localStorage.removeItem(key);
            }
        }
    });
    prefetchPricingServer();
    autoTranslate();
    pageUpdated = !0
}

export function revertDefaultSettingsFunc() {
    ...
    if (btn) {
        ...
        function textOfLabelForInput(input) {
            ...
            if (!label) {
                if (input.id) {
                    const l = document.querySelector('label[for="' + (CSS && CSS.escape ? CSS.escape(input.id) : input.id) + '"]');
        ...
        function updateSliderValue(range) {
            ...
            if (!box) return;
            const val = box.querySelector('.slider-value');
            if (val) val.textContent = range.value;

        }
        /* ---------- defaults restore logic ---------- */
        function applyDefaultToElement(el) {
            const raw = el.getAttribute('default');
            if (raw == null) return;
            ...
            const defaults = raw.split(',').map(s => s.trim()).filter(Boolean);
            const inputs = Array.from(combobox.querySelectorAll('input[type="checkbox"]'));

            if (!inputs.length) return;

            inputs.forEach(inp => {
                const match =
                    defaults.includes(inp.id) ||
                    defaults.includes(inp.value) ||
                    defaults.includes(textOfLabelForInput(inp));
        ...
        function resetSettingsToDefaults() {
            const nodes = Array.from(document.querySelectorAll('[default]'));
            nodes.forEach(n => applyDefaultToElement(n));
            document.dispatchEvent(new CustomEvent('settings:reset', { bubbles: true }));
        }

        function isCheckboxAtDefault(input) {
            const b = parseDefaultBool(input.getAttribute('default'));
            return b === null || input.checked === b;
        }

        function isRangeAtDefault(input) {
            return String(input.value) === String(input.getAttribute('default'));
        }

        function isComboboxAtDefault(el) {
            const combobox = el.closest('.combobox');
            if (!combobox) return true;

            // Check checkboxes
            const checkboxes = combobox.querySelectorAll('input[type="checkbox"][default]');
            const checked = Array.from(checkboxes).filter(cb => cb.checked);

            if (checkboxes.length > 0) {
                if (checked.length === 0) {
                    const allDefaultsFalse = Array.from(checkboxes).every(cb => parseDefaultBool(cb.getAttribute('default')) === false);
                    if (!allDefaultsFalse) return false;
                } else {
                    const checkboxesMatch = Array.from(checkboxes).every(cb => {
                        const def = parseDefaultBool(cb.getAttribute('default'));
                        return def === null || cb.checked === def;
                    });
                    if (!checkboxesMatch) return false;
                }
            }

            // Check textareas
            const textareas = combobox.querySelectorAll('textarea[default]');
            const textareasAtDefault = Array.from(textareas).every(ta => {
                const def = ta.getAttribute('default');
                return (def === 'empty_textarea' && ta.value === '') || ta.value === def;
            });
            if (!textareasAtDefault) return false;

            // Fallback: normal combobox text
            if (checkboxes.length === 0 && textareas.length === 0) {
                const span = combobox.querySelector('.combobox-text[default]');
                if (span) {
                    let currentText = (span.textContent || '').trim();

                    // Remove prefix if exists (e.g., "Change: " or "Keep: ")
                    const colonIndex = currentText.indexOf(':');
                    if (colonIndex !== -1) {
                        currentText = currentText.slice(colonIndex + 1).trim();
                    }

                    const def = span.getAttribute('default');
                    if (currentText !== def) return false;
                }
            }
        ...
        function settingsAreAtDefaults(debug = false) {
            const nodes = document.querySelectorAll('[default]');
            const notDefault = [];
            ...
            for (const el of nodes) {
                ...
                // ---------- COMBOBOX ----------
                if (el.closest('.combobox')) {
                    if (!isComboboxAtDefault(el)) {
                        const combobox = el.closest('.combobox');
                        const checked = Array.from(
                            combobox.querySelectorAll('input[type="checkbox"]:checked')
                        ).map(i => textOfLabelForInput(i));

                        notDefault.push({
                            type: 'combobox',
                            id: combobox.id || '(no id)',
            ...
            if (debug && notDefault.length) {
                console.groupCollapsed(
                    `⚠️ Settings not at default (${notDefault.length})`
                );
                notDefault.forEach(item => {
                    console.log(
                        `[${item.type}]`,
                        `id: ${item.id}`,
        ...
        function updateResetButtonState() {
            ...
            const isDefault = settingsAreAtDefaults();
        ...
        document.addEventListener('input', updateResetButtonState);
        document.addEventListener('change', updateResetButtonState);
...
const progressMap = window.progressMap || {}; // assume you already have progressMap globally
// lastProgress, pageName etc are assumed to exist in your environment

// --- Download cancel / notification API ---------------------------------
export function getDownloadCancelled(id) {
    if (typeof id !== 'undefined' && id !== null) {
        return !!downloadCancelledMap.get(id);
    }
    return !!globalDownloadCancelled;
}

/**
 * setDownloadCancelled(newValue, id?)
 * - if id provided: set per-download state and abort/resume only that controller
 * - if id omitted: act globally (set global flag and abort all controllers)
 */
export function setDownloadCancelled(newValue, id, showNotification = false) {
    const v = !!newValue;

    if (typeof id !== 'undefined' && id !== null) {
        downloadCancelledMap.set(id, v);

        const message = v
            ? `Download ${id} has been cancelled.`
            : `Download ${id} has been resumed.`;

        try { if (showNotification) showNotification(message, 'Download', 'warning'); } catch (e) { console.warn('showNotification missing', e); }

        const ac = abortControllers.get(id);
        if (v) {
            if (ac && !ac.signal.aborted) {
                console.warn(`[setDownloadCancelled] aborting controller for ${id}`);
                try { ac.abort(); } catch (e) { /* no-op */ }
            }
        } else {
            // create a fresh controller for resume (so old aborted signal isn't reused)
    ...
    // Global behaviour: set global flag and abort all controllers if cancelling
    globalDownloadCancelled = v;
    try { showNotification(v ? 'All downloads cancelled.' : 'Downloads resumed.', 'Download', 'warning'); } catch (e) { }

    if (v) {
        for (const [k, ac] of abortControllers.entries()) {
            if (ac && !ac.signal.aborted) {
                try {
                    console.warn(`[setDownloadCancelled global] aborting controller for ${k}`);
                    ac.abort();
                } catch (e) { /* ignore */ }
            }
            downloadCancelledMap.set(k, true);
        }
    } else {
        // resume globally: create fresh controllers for all known IDs
        for (const k of abortControllers.keys()) {
            abortControllers.set(k, new AbortController());
            downloadCancelledMap.set(k, false);
        }
    }
}
...
/**
 * getAbortController() - legacy single controller (kept for compatibility)
 * returns a global controller; prefer getAbortControllerFor(id)
 */
let legacyAbortController = null;
export const getAbortController = () => {
    if (!legacyAbortController) legacyAbortController = new AbortController();
    return legacyAbortController;
};

/**
 * getAbortControllerFor(id)
 * returns an AbortController for a given download id (creates if none)
 */
export function getAbortControllerFor(id) {
    if (!id && id !== 0) {
 ...
 * removes controller for id (cleanup)
 */
export const resetAbortControllerFor = (id) => {
    if (!id && id !== 0) return;
    abortControllers.delete(id);
    downloadCancelledMap.delete(id);
};

export function deleteDownloadData(timestamp, id, url = undefined) {
    ...
    if (url !== undefined)
        localStorage.removeItem(url + '_formData');
    ...
    const abortController = getAbortControllerFor(id);
    if (abortController) {
...
// --- handleDownload (full fixed function) --------------------------------
export const handleDownload = async ({
    ...
    console.log('[handleDownload] called from:\n', new Error().stack);
    ...
    element.innerHTML = `<initial url="${url}" id="${id}" timestamp="${timestamp}" active="${active}"><div class="process-text">Downloading...</div><div class="delete-icon"></div><div class="settings-mark"></div></initial>`;
    ...
    signal.addEventListener('abort', () => {
        console.warn(`[abort event] signal aborted for id=${id}, timestamp=${timestamp}`, new Error().stack);
    });
    ...
    while (!downloadCancelledMap.get(id) && !signal.aborted) {
        try {
            ...
            if (![200, 206, 416].includes(res.status)) {
                ...
                deleteDownloadData(timestamp, id, url);
            ...
            totalBytes ||= parseInt(contentLength) || 0;
            ...
            // inner streaming loop (read chunks)
            while (!downloadCancelledMap.get(id) && !signal.aborted) {
                let readResult;
                try {
                    ...
                    if (err && err.name === 'AbortError') {
                        console.warn(`[reader aborted] id=${id}`, err);
                        processText(`Paused: Operation was aborted`);
                        return;
                    }
                    throw err;
                }
                ...
                if (Math.floor(lastProgress) % 10 === 0 && Math.floor(lastProgress) > lastSavedProgress) {
                    await appendChunkToDB(db, url, value).catch(e => console.warn('appendChunkToDB failed', e));
            ...
            // persist final blob + bookkeeping
...
const replaceElementForData = async (el, dataBaseObjectStoreName, data = {}) => {
    const { blob, base64, url, id, timestamp, active } = data;

    if (!el) throw new Error('No element provided');

    let mime = null;
    if (blob instanceof Blob) mime = blob.type;
    else if (typeof base64 === 'string' && base64.startsWith('data:')) {
        const m = /^data:([^;]+);/.exec(base64);
        mime = m ? m[1] : null;
    }

    const desiredTag = mime?.startsWith('video/') ? 'video' : 'img';
    const currentTag = el.tagName.toLowerCase();

    let newEl = el;

    if (desiredTag !== currentTag) {
        showNotification('Replacing the element format.', 'Loading in progress', 'normal');

        if (desiredTag === 'video') {
            newEl = document.createElement('video');
            newEl.setAttribute('url', url || '');
            newEl.setAttribute('timestamp', timestamp || '');
            newEl.setAttribute('active', active ? 'true' : 'false');
            newEl.setAttribute('resized', !active ? 'true' : 'false');
            newEl.id = id;
            newEl.muted = true;
            ...
            try { newEl.disablePictureInPicture = true; } catch { }
            ...
            if (dataBaseObjectStoreName === 'inputs') {
                ...
                function handleMetadataUpdate(dataFps, dataDuration) {
                    const tooltip = newEl.querySelector('.tooltip');
                    if (tooltip && dataFps) {
                        tooltip.style.display = 'flex';
                        const fpsSliderValue = keepFPS && !keepFPS.checked ? fpsSlider.value : 60;
                        const fps = Math.min(fpsSliderValue, dataFps);
                        ...
                        tooltip.textContent = pageName === 'art-generator' ? 'Free!' : `${neededCredits} Credits`;
                    }
                }

                calculateMetadata(newEl, handleMetadataUpdate);
                newEl.addEventListener('loadedmetadata', async () => {
                    await calculateMetadata(newEl, handleMetadataUpdate);
                });

                [keepFPS, fpsSlider, removeBanner].forEach(el => {
                    if (el) {
                        el.addEventListener('change', async () => {
                            const dataFps = newEl.getAttribute('data-fps');
                            handleMetadataUpdate(dataFps, newEl.duration);
                        });
                    }
                });
            }
            ...
            if (dataBaseObjectStoreName === 'inputs' && pageName === 'face-swap') {
                const removeBanner = document.getElementById("removeBanner");
                function handleMetadataUpdate() {
                    const t = newEl.querySelector('.tooltip');
                    if (t) {
                        t.style.display = 'flex';
                        let neededCredits = removeBanner && removeBanner.checked ? 2 : 1;
                        t.textContent = `${neededCredits} Credits`;
...
/*const loadMedia = async (el, data, opts = {}) => {
    if (!el) throw new Error('No element provided');
    ...
    if (!['img', 'video'].includes(tag)) throw new Error('Element must be <img> or <video>');
    ...
    console.log('=== LOAD MEDIA DIAGNOSTICS START ===');
    console.log('Device:', diagnostics.deviceInfo);
    console.log('Blob Info:', diagnostics.blobInfo);
    ...
    try {
        ...
        console.warn('Failed to clear src:', e);
    }

    if (tag === 'video' && data.blob) {
        const codecChecks = {
            'video/mp4; codecs="avc1.42E01E"': el.canPlayType('video/mp4; codecs="avc1.42E01E"'),
            'video/mp4': el.canPlayType('video/mp4'),
            'video/webm': el.canPlayType('video/webm'),
            [data.blob.type]: el.canPlayType(data.blob.type)
        };
        diagnostics.codecSupport = codecChecks;
        console.log('Codec Support:', codecChecks);

        try {
            const firstBytes = await data.blob.slice(0, 12).arrayBuffer();
            const bytes = new Uint8Array(firstBytes);
            const hex = Array.from(bytes).map(b => b.toString(16).padStart(2, '0')).join(' ');
            diagnostics.blobSignature = hex;
            console.log('Blob signature (first 12 bytes):', hex);

            const isMP4 = bytes[4] === 0x66 && bytes[5] === 0x74 && bytes[6] === 0x79 && bytes[7] === 0x70;
            const isWebM = bytes[0] === 0x1A && bytes[1] === 0x45 && bytes[2] === 0xDF && bytes[3] === 0xA3;
            diagnostics.formatDetected = isMP4 ? 'MP4' : (isWebM ? 'WebM' : 'Unknown');
            console.log('Format detected:', diagnostics.formatDetected);
        } catch (e) {
            diagnostics.blobReadError = e.message;
            console.error('Failed to read blob bytes:', e);
        }
    }

    const attemptLoadSrc = (src, attemptNum) => {
        return new Promise((resolve, reject) => {
            const attemptLog = {
                attemptNumber: attemptNum,
                srcType: src.startsWith('blob:') ? 'blob' : 'base64',
            ...
            if (signal?.aborted) {
                attemptLog.finalError = 'Aborted before start';
            ...
            const logEvent = (eventName, details = {}) => {
                attemptLog.events.push({
                    time: Date.now() - attemptLog.startTime,
                    event: eventName,
                    ...details
                });
            };
            ...
            const fail = (err) => {
                attemptLog.finalError = err.message;
                attemptLog.duration = Date.now() - attemptLog.startTime;
                attemptLog.finalElementState = getElementState();
                diagnostics.attempts.push(attemptLog);
                cleanup();
                reject(err);
            };
            ...
            const getElementState = () => {
                ...
                if (tag === 'video') {
                    ...
                    state.currentSrc = el.currentSrc?.substring(0, 60);
                    if (el.error) {
                        state.error = {
                            code: el.error.code,
                    ...
                    state.complete = el.complete;
                    state.naturalWidth = el.naturalWidth;
                    state.naturalHeight = el.naturalHeight;
                }

                return state;
            };

            const onLoadStart = () => logEvent('loadstart', getElementState());
            const onProgress = () => logEvent('progress', getElementState());
            const onSuspend = () => logEvent('suspend', getElementState());
            const onStalled = () => logEvent('stalled', getElementState());

            const onLoadedImage = () => {
                logEvent('load', getElementState());
                if (el.naturalWidth === 0) {
                    return fail(new Error('Image loaded but naturalWidth is zero'));
                }
                succeed();
            };

            const onLoadedMetadata = () => {
                logEvent('loadedmetadata', getElementState());
            };

            const onLoadedData = () => {
                logEvent('loadeddata', getElementState());
                if (el.readyState >= 2) {
                    succeed();
                }
            };
            ...
            const onCanPlay = () => {
                logEvent('canplay', getElementState());
                succeed();
            };

            const onCanPlayThrough = () => {
                logEvent('canplaythrough', getElementState());
                succeed();
            };
            ...
            const onError = (e) => {
                ...
                fail(new Error(`Media error: ${el.error?.code} - ${el.error?.message || 'Unknown'}`));
            };

            if (tag === 'img') {
                el.onload = onLoadedImage;
                el.onerror = onError;
                el.src = src;
                ...
                if (typeof el.decode === 'function') {
                    el.decode()
                        .then(() => {
                            logEvent('decode_success');
                            onLoadedImage();
                        })
                        .catch(err => {
                            logEvent('decode_failed', { error: err.message });
                        });
                }
            } else {
                ...
                // CRITICAL FIX: Set src and call load() IMMEDIATELY, no setTimeout
                el.src = src;
                //el.poster = '';

                try {
                    logEvent('calling_load');
                    el.load();
                } catch (e) {
                    logEvent('load_threw_error', { error: e.message });
                    fail(e);
                }
            }

            timeoutId = setTimeout(() => {
                fail(new Error(`Timeout after ${timeoutPerAttempt}ms`));
            }, timeoutPerAttempt);
    ...
    while (true) {
        ...
        if (maxAttempts && attempt > maxAttempts) {
            ...
            console.log('=== LOAD MEDIA DIAGNOSTICS END (FAILED) ===');
            console.log('Full diagnostics:', diagnostics);

            const error = new Error(`Failed to load after ${maxAttempts} attempts`);
        ...
        if (data.blob instanceof Blob) {
            ...
            try {
                ...
                // Try to read first byte to ensure blob is accessible
                try {
                    const testRead = await data.blob.slice(0, 1).arrayBuffer();
                    if (testRead.byteLength === 0) {
                        throw new Error('Blob is not readable');
                    }
                } catch (readErr) {
                    throw new Error('Blob read test failed: ' + readErr.message);
                }

                lastBlobUrl = URL.createObjectURL(data.blob);
                console.log('[loadMedia] Created blob URL:', lastBlobUrl, 'for blob size:', data.blob.size);

                await attemptLoadSrc(lastBlobUrl, attempt);
                ...
                console.log('=== LOAD MEDIA DIAGNOSTICS END (SUCCESS) ===');
                console.log('Success diagnostics:', diagnostics);
        ...
        if (typeof data.base64 === 'string' && data.base64.startsWith('data:')) {
            try {
                await attemptLoadSrc(data.base64, attempt);
                ...
                lastError = err;
                console.warn(`Attempt ${attempt} (base64) failed:`, err.message);
            }
        }

        const jitter = Math.round(Math.random() * 200);
        const waitTime = Math.min(maxDelay, Math.round(delay) + jitter);
        await sleep(waitTime);
        delay = Math.min(maxDelay, delay * backoffFactor);
    }
};*/

const removeMedia = async (el) => {
    if (!el) return;

    // Already detached / missing
    ...
    try {
        ...
        // Wait until media is actually paused if it can report state
        ...
        // Force load reset (important!)
        try {
            el.load();
        } catch { }

        // Wait for browser to settle network state → EMPTY
        await new Promise((resolve) => {
            // If already empty, resolve immediately
            if (el.networkState === el.NETWORK_EMPTY) {
                return resolve();
            }

            // Fallback timer in case events never fire
        ...
        // Remove any src children (<source>)
        ...
        console.warn("removeMedia error:", err);
    }
};


let lastClickTime = 0;
let isMediaLoading = {}; // <-- object-based lock

async function isDebug() {
    let userDoc = await getUserDoc();
    return userDoc.username == 'durieun02';
}

export const handleFileContainerEvents2 = async (event, dataBaseIndexName, dataBaseObjectStoreName, container, databases) => {
    const now = Date.now();
    ...
    // Use per-store loading lock
    if (isMediaLoading[dataBaseObjectStoreName]) {
        showNotification('Please wait until current media finishes loading.', 'Loading in progress', 'warning');
        return;
    }
    ...
    if (event.target.classList.contains('settings-mark')) {
        return;
    }
    ...
    // Show loader BEFORE any async operations
    if (pageName === 'inpaint' || pageName !== 'art-generator' || pageName === 'art-generator' || pageName === 'face-swap' || dataBaseObjectStoreName === 'outputs') {
        //await displayStoredData('loader', pageName === 'inpaint' ? 'outputs' : dataBaseObjectStoreName);
    }

    let db = openDB(dataBaseIndexName, dataBaseObjectStoreName);

    const elements = [...container.querySelectorAll('.data-container img, .data-container video')]
        .filter(element => !element.classList.contains('active'));

    for (const element of elements) {
        try {
            const originalSrc = element.src;
            if (element.getAttribute('resized') === 'false') {
                //replaceWithLowRes(element);
                element.setAttribute('resized', 'true');
            }
            if (originalSrc && originalSrc.startsWith('blob:')) {
                //URL.revokeObjectURL(originalSrc);
            }
        } catch (err) {
            console.error('Failed to process element:', err);
        }
    }
    ...
    if (parent.classList.contains('active')) {
        ...
        if (pageName === 'inpaint' || pageName !== 'art-generator' || pageName === 'face-swap' || dataBaseObjectStoreName === 'outputs') {
            await displayStoredData(null, pageName === 'inpaint' ? 'outputs' : dataBaseObjectStoreName);
        }
        ...
        if (element.src?.startsWith('blob:')) {
            if (element.tagName.toLowerCase() === 'video') element.pause();
            URL.revokeObjectURL(element.src);
        }
        return;
    }
    ...
    const activeEls = [...container.querySelectorAll('.data-container.active')];
    for (const activeEl of activeEls) {
        activeEl.classList.remove('active');

        const element = activeEl.querySelector('img, video, iframe, initial');
        if (element?.tagName.toLowerCase() === 'iframe') {
            element.style.width = 'calc(var(--input-size))';
            element.style.height = 'calc(var(--input-size))';
        }
    ...
    element.setAttribute('resized', 'false');
    ...
    if (!isNaN(idx)) {
        await updateActiveState(db, idx, true);
        element = await removeMedia(element);

        const items = await getFromDB(db);
        let item = items.find(item => item.id === idx);
        ...
        console.log('[handleFileContainerEvents] Found item:', {
            id: item.id,
            hasBlob: !!item.blob,
        ...
        try {
            isMediaLoading[dataBaseObjectStoreName] = true;

            console.log('[handleFileContainerEvents] Replacing element...');
            //element = await replaceElementForData(element, dataBaseObjectStoreName, item);

            console.log('[handleFileContainerEvents] Loading media...');
            //loadedElement = await loadMedia(element, item);
            console.log('[handleFileContainerEvents] Media loaded successfully');

            // Step 5: NOW call displayStoredData - media is already loaded
            if (pageName === 'inpaint' || pageName === 'art-generator' || pageName === 'face-swap' || dataBaseObjectStoreName === 'outputs') {
                console.log('[handleFileContainerEvents] Displaying stored data...');
                showNotification('Loading the data to display section.', 'Loading in progress', 'normal');
                await displayStoredData(parent, pageName === 'inpaint' ? 'outputs' : dataBaseObjectStoreName);
                showNotification('Loaded the data to display section.', 'Loading complete', 'normal');
            }
            console.log('[handleFileContainerEvents] Complete!');

        } catch (err) {
            console.error('[handleFileContainerEvents] ERROR:', err);

            let msg = '=== LOADING FAILED ===\n\n';
            msg += 'Error: ' + err.message + '\n\n';

            msg += '--- BLOB INFO ---\n';
            msg += 'Has Blob: ' + (item?.blob ? 'YES' : 'NO') + '\n';
            if (item?.blob) {
                msg += 'Size: ' + Math.round(item.blob.size / 1024) + ' KB\n';
                msg += 'Type: ' + item.blob.type + '\n';
            }
            msg += 'Has Base64: ' + (item?.base64 ? 'YES' : 'NO') + '\n\n';

            msg += '--- ELEMENT STATE ---\n';
            msg += 'Tag: ' + element?.tagName + '\n';
            msg += 'Ready: ' + element?.readyState + '\n';
            msg += 'Network: ' + element?.networkState + '\n';

            if (element?.error) {
                msg += '\n*** MEDIA ERROR ***\n';
                msg += 'Code: ' + element.error.code + '\n';
                const errorNames = {
                    1: 'ABORTED',
                    2: 'NETWORK',
                    3: 'DECODE',
                ...
                msg += 'Type: ' + (errorNames[element.error.code] || 'UNKNOWN') + '\n';
            }

            if (err.diagnostics?.codecSupport) {
                msg += '\n--- CODEC SUPPORT ---\n';
                const cs = err.diagnostics.codecSupport;
                msg += 'MP4: ' + (cs['video/mp4'] || 'NO') + '\n';
                msg += 'H.264: ' + (cs['video/mp4; codecs="avc1.42E01E"'] || 'NO') + '\n';
                msg += 'WebM: ' + (cs['video/webm'] || 'NO') + '\n';
                msg += 'Blob type support: ' + (cs[item?.blob?.type] || 'NO') + '\n';
            }

            if (err.diagnostics?.formatDetected) {
                msg += '\nDetected: ' + err.diagnostics.formatDetected + '\n';
            }

            if (err.diagnostics?.blobSignature) {
                msg += 'Signature: ' + err.diagnostics.blobSignature + '\n';
            }

            if (err.diagnostics?.attempts?.length > 0) {
                msg += '\n--- ATTEMPTS ---\n';
                err.diagnostics.attempts.forEach((a, i) => {
                    msg += 'Attempt ' + (i + 1) + ': ';
                    msg += a.events.map(e => e.event).join(', ') + '\n';
                    if (a.finalError) msg += '  Error: ' + a.finalError + '\n';
                });
            }

            msg += '\n--- DEVICE ---\n';
            msg += navigator.userAgent.substring(0, 100) + '...\n';

            console.log('DIAGNOSTIC REPORT:', msg);
            alert(msg);
            showNotification('Loading failed - check alert', 'Error', 'error');
        } finally {
            isMediaLoading[dataBaseObjectStoreName] = false;
        }
    }
};
...
export async function showFrameSelector(element) {
    ...
    function resetToInputContainer() {
        const outputContainer = wrapper.querySelector('[query="outputContainer"]');
        if (outputContainer)
            outputContainer.style.display = "none";

        const inputContainer = isVideo
            ? wrapper.querySelector('[query="videoContainer"]')
            : wrapper.querySelector('[query="imgContainer"]');
    ...
    async function handleProcess() {
        ...
        function getSelectedPerson(disable = true) {
        ...
        function getSelectedGender(disable = true) {
        ...
        function getSelectedRace(disable = true) {
        ...
        try {
            ...
            if (typeof userData !== 'object' || !userData?.uid) {
                ...
                getSelectedPerson(false);
                getSelectedRace(false);
                getSelectedGender(false);
                return;
            }
            ...
            async function getServerAddresses() {
                try {
                    ...
                    let addresses = await fetchServerAddresses(snapshotPromise, keepSlowServers, null, true);
                    if (!addresses || !Array.isArray(addresses) || addresses.length === 0) {
                        console.warn('[getServerAddresses] Cached version failed, retrying without cache...');
                        addresses = await fetchServerAddresses(snapshotPromise, keepSlowServers, null, false);
                    }

                    if (!addresses || !Array.isArray(addresses) || addresses.length === 0) {
                        showNotification(`No server addresses available.`, 'Warning', 'warning-important');
                        return null;
                    }

                    return addresses;
                } catch (error) {
                    console.error('[getServerAddresses] Failed to get server addresses:', error);
            ...
            const isolatedServers = new Set([].map(s => s.toLowerCase()));
            ...
            const interval = setInterval(async () => {
                ...
                if (data?.status !== 'processing') {
                    ...
                    if (data?.status === 'completed') {
                        ...
                        imgElement.setAttribute('query', 'outputContainer');
...
async function supportsWebP() {
    if (_webpSupported !== undefined) return _webpSupported;
    try {
        ...
        await new Promise((res, rej) =>
        (c.convertToBlob
            ? c.convertToBlob({ type: 'image/webp' }).then(res).catch(rej)
            : c.toBlob(b => b ? res() : rej(), 'image/webp'))
        );
        _webpSupported = true;
    } catch {
...
async function createThumbnail(fileOrBlob, arrayBuffer = null, options = {}) {
    const { maxWidth = 128, maxHeight = 128, quality = 0.5 } = options;
    ...
    // Helper: draw an image or video frame to a resized canvas and convert to Blob
    async function drawToBlob(sourceWidth, sourceHeight, drawFn) {
        ...
        return canvas.convertToBlob
            ...
            : await new Promise((res) => canvas.toBlob(res, outputType, quality));
    }
    ...
    if (mimeType.startsWith('image/')) {
        ...
        if (arrayBuffer) {
            ...
            throw new Error('Invalid input for image thumbnail');
        }

        const thumbBlob = await drawToBlob(bitmap.width ?? bitmap.naturalWidth, bitmap.height ?? bitmap.naturalHeight, (ctx, w, h) =>
            ctx.drawImage(bitmap, 0, 0, w, h)
        );

        // Only return if smaller than original
        const originalSize = arrayBuffer ? arrayBuffer.byteLength : fileOrBlob.size;
        return thumbBlob && thumbBlob.size < originalSize ? thumbBlob : null;
    }
    ...
    if (mimeType.startsWith('video/')) {
        return new Promise((resolve, reject) => {
            ...
            video.onseeked = async () => {
                try {
                    const thumbBlob = await drawToBlob(video.videoWidth, video.videoHeight, (ctx, w, h) =>
                        ctx.drawImage(video, 0, 0, w, h)
                    );
                    cleanup();
                    resolve(thumbBlob && thumbBlob.size < fileOrBlob.size ? thumbBlob : null);
                } catch (err) {
                    cleanup();
                    reject(err);
                }
...
async function replaceWithLowRes(el, scale = 0.25, quality = 0.5) {
    if (!el.src) return;
    ...
    await new Promise(resolve => {
        if (el.tagName.toLowerCase() === 'img') {
            ...
            el.addEventListener('error', resolve, { once: true }); // resolve even if broken
        } else if (el.tagName.toLowerCase() === 'video') {
    ...
    if (el.tagName.toLowerCase() === 'img') {
        canvas.width = el.naturalWidth * scale;
        canvas.height = el.naturalHeight * scale;
        ctx.drawImage(el, 0, 0, canvas.width, canvas.height);

        const lowResUrl = await new Promise(resolve =>
            canvas.toBlob(blob => resolve(URL.createObjectURL(blob)), 'image/jpeg', quality)
        );

        el.src = lowResUrl;
    } else if (el.tagName.toLowerCase() === 'video') {
        canvas.width = el.videoWidth * scale;
        canvas.height = el.videoHeight * scale;
        ctx.drawImage(el, 0, 0, canvas.width, canvas.height);
                ...
                reader.onload = async (e) => {
                    try {
                        ...
                        try {
                            ...
                            // thumb remains null, DB update can still proceed
                        }
...
function updateGenerateButtonText() {
    ...
    if (pageName === 'video-generator' || pageName === 'inpaint' || pageName === 'art-generator') {
        ...
        if (handleProcessText) {
            handleProcessText.textContent = pageName === 'art-generator' ? 'Generate (Free!)' : `Generate (${neededCredits} Credit${neededCredits > 1 ? 's' : ''})`;
...
function setProcessingState(state) {
    ...
    isProcessing = (state === true || state === undefined) ? true : false;
    if (state === undefined) {
        handleProcessText.textContent = 'Loading...';
        handleProcessBtn.disabled = true;
        return;
    }
...
export async function checkServerStatus(databases = null, userId = null) {
    const dbKey = `${pageName}_databases`;
    const userKey = `${pageName}_userId`;

    if (databases) 
        localStorage.setItem(dbKey, JSON.stringify(databases));

    if (userId) 
        localStorage.setItem(userKey, userId);

    const dbKeyStored = localStorage.getItem(dbKey);
    const userKeyStored = localStorage.getItem(userKey);

    if (!didCallStatus) {
        if (!databases && dbKeyStored)
            databases = JSON.parse(dbKeyStored);

        if (!userId && userKeyStored)
            userId = userKeyStored;
    }
    ...
    let serversAdresses = getFetchableServerAdresses();
    if (!serversAdresses || !serversAdresses.length) {
        try {
            const snapshotPromise = () => getDocsSnapshot('servers');
            setFetchableServerAdresses((await fetchServerAddresses(snapshotPromise)).reverse());
            serversAdresses = getFetchableServerAdresses();
        } catch (error) {
    ...
    if (!serversAdresses) return;

    const serverPromises = serversAdresses.map(async (server) => {
        try {
            ...
            if (data.onlineStatus && server) {
                const url = new URL(server);
                localStorage.setItem(`onlineStatus_${url.origin}`, data.onlineStatus);
            }
        ...
        allCheckboxes.forEach(checkbox => {
            ...
            if (isActive) {
                ...
                if (!wrapper) {
                    ...
                    wrapper.classList.add('model-container');

                    const modelSpan = parent.querySelector('.model-name');
                    if (modelSpan) wrapper.appendChild(modelSpan);

                    parent.appendChild(wrapper);
                }

                if (!wrapper.querySelector('.status-dot')) {
    ...
    if (serverWithUserRequest) {
        ...
        if (downloadFile) {
            ...
            if (data?.status === 'completed') {
                let elapsedMs = performance.now() - processStartTime;
                let elapsedSec = (elapsedMs / 1000).toFixed(2);

                //console.log(`⏱️ Process completed in ${elapsedSec} seconds`);

                showNotification(
                    `Request ${data?.status} With Status ${data?.server}. Took ${elapsedSec}s.`,
                    'Fetch Information',
                    'default'
                );
...
export const setDynamicInterval = (databases, userId) => {
    ...
    let waitTime = downloadFile ? 1000 : 10000;
    if (pageName === 'art-generator')
        waitTime = downloadFile ? 500 : 1000;
    intervalId = setInterval(() => checkServerStatus(databases, userId), waitTime)
};
...
export function findServerWithUserRequest(results, userId) {
    const match = results.find(serverData =>
        serverData.requestQueue?.includes(userId)
    );
    return match;
}
    ...
    const applyCommonLogic = (containerSelector, options = {}) => {
        ...
        containers.forEach(container => {
            ...
            const sliderInput = container.querySelector(
                "input[type='range']:not(.list-items input[type='range'])"
            );
            ...
            if (options.selectBtnSelector === '.combobox') {
                ...
                if (!hasTextarea) {
                    items.forEach(item => {
                        ...
                        if (checkbox && checkbox.checked) {
                            ...
                            btnText.innerText = title + ": " + (nameSpan ? nameSpan.textContent.trim() : '');
                        } else {
                ...
                items.forEach(item => {
                    ...
                    const updateBtnText = () => {
                        ...
                        const selectedNames = Array.from(checkedItems).map(cb => {
                            const label = cb.parentElement.querySelector('span');
                            if (!label) return '';
                            return label.getAttribute('short')?.trim() || label.textContent.trim();
                        }).filter(name => name !== '');
                        btnText.innerText = selectedNames.length > 0 ? btnText.getAttribute("title") + ": " + selectedNames.join(', ') : btnText.getAttribute("title");
                    };
            ...
            const toggleOpen = (event) => {
                ...
                if (isCurrentlyOpen) {
                    if (listItems && !listItems.contains(event.target)) {
                ...
                document.querySelectorAll(`.multibox.open, .combobox.open, .rectangle.open`).forEach(openContainer => {
                    if (openContainer === container) return;
                    const otherListItems = openContainer.querySelector(".list-items");
                    if (otherListItems) otherListItems.style.display = "none";
                    openContainer.classList.remove("open");
                });
...
export async function cancelProcess(showAlertion) {
    try {
        const userData = await getUserData();
        const hasId = pageName !== 'art-generator' ? true : false;
        const userId = hasId ? userData?.uid : localStorage.getItem('guestId');
        const processToken = localStorage.getItem('userToken');

        let serverWithUserRequest = getProcessingState();
        if (!serverWithUserRequest && hasId) {
            const userDoc = await getUserDoc(() => setCurrentUserDoc(getDocSnapshot));
        ...
        if (serverWithUserRequest) {
            ...
            console.error(`User is not processing. No cancellation request sent.`);
        }
    } catch (error) {
        showNotification(error.message, 'Warning - Process Cancellation', 'warning');
...
window.alert = function (message) {
    try {
        ...
        console.log('Alert triggered:', message, '(failed to get stack)');
    }
    originalAlert(message);
};

Object.defineProperty(window, 'downloadCancelled', {
    get() { return _downloadCancelled; },
    set(v) { console.log('[downloadCancelled] set to', v, new Error().stack); _downloadCancelled = v; }
});