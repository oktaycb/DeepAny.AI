...
export function loadPageContent(updateMainContent) {
	...
	let scaleFactor = parseFloat(localStorage.getItem('scaleFactor')) || 0.7;
		...
		if (screenMode !== 1) {
			...
			if (!navLinks || navLinks.length === 0) {
				navContainer.insertAdjacentHTML('beforeend', `
						<ul class="nav-links" style="display: grid;grid-template-columns: 2fr 1fr 2fr;justify-items: center;">
							<li>
								...
								<ul class="dropdown-menu">
									...
									<li><a class="text" href="art-generator.html">Art</a></li>
								</ul>
							</li>
							<li>
						...
						<div class="nav-links" style="display: flex;justify-content: center;gap: clamp(0px, calc(1vh * var(--scale-factor)), calc(12vw * var(--scale-factor)));">
	...
	var link = document.getElementById('loading-stylesheet');
	if (link)
